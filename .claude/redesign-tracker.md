...
| F7 | `/courses/[subject]` (all 4) | **Read times are hand-typed and understated up to 2.3×** vs the markdown (200 wpm). Organic Chem U2 says 16 min, 7,524 words ≈ 37 min; subject total 57 → ≈111 min; Biochem 69 → ≈86 (17,328 words) | `wc -w content/**` (was `public/content/` until 2026-09-23) | P2 courses — decision needed: derive from word count (data change in `subjects/*.ts`) or relabel |
...
| Status | Page | File | Notes |
| --- | --- | --- | --- |
| [x] done | `/about-us` | rebuilt 2026-09-16 (twice) | **Now the P9 static-page language**: the GSAP roster-stage version was replaced the same day at the user's request by a simple page-kit page — hero, story, pillar cards, then leadership feature cards + flip cards under sticky group labels, 17 people, monograms only. CDP checks pass (hover flip verified by CSS only) |
| [ ] todo | `/faqs` | (246) | |
| [ ] todo | `/careers` | (206) | |
| [ ] todo | `/mentor` | (343) | |
