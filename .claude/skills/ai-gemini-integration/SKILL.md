...
## Read First
- `src/app/api/chat/route.ts` — the `SYSTEM_PROMPT` and history-shaping rules.
...
| --- | --- | --- | --- |
| Chat tutor | `/api/chat` | `@google/generative-ai` (`GoogleGenerativeAI`) | Rebuilds alternating user/model history; must start with `user` |
...
**None of the four routes is authenticated or rate limited.** They are public endpoints that spend
money per call. This is a tracked `ROADMAP.md` Phase 5 item. If you add a fifth AI route, add a
rate limit — do not extend the pattern.
...
curl -s -X POST localhost:3000/api/chat \
  ...
  -d '{"messages":[{"role":"user","content":"What is the MOA of metformin?"}]}'
```
Requires a real `GEMINI_API_KEY` and **costs money per call** — test sparingly. Also test the
no-key path by temporarily unsetting the variable (it must fail cleanly, not crash). Verify the
prompt's safety steering by asking something off-topic and something patient-specific.
**No tests exist**, and model output is non-deterministic — exercise it, describe what you saw, and
do not claim deterministic correctness.
