...
import { Calculator, Thermometer, AlertCircle, RefreshCw, Flame, Zap } from 'lucide-react';
...
export default function HeatFormationCalculator() {
  ...
  const calculateEnthalpy = () => {
    ...
    if (!isNaN(S1) && !isNaN(S2) && !isNaN(T1) && !isNaN(T2) && 
      ...
      const deltaH = 2.303 * R * 
        (Math.log10(S2) - Math.log10(S1)) * 
        (T1K * T2K) / (T2K - T1K);
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 p-4 md:p-6 mt-16 md:mt-20">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-orange-600 to-red-500 rounded-2xl shadow-xl p-6 md:p-8 mb-6 md:mb-8">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="flex items-center mb-4 md:mb-0">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-white">Heat of Formation Calculator</h1>
                <p className="text-orange-100 mt-2">Calculate ΔH from solubility-temperature relationship</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 bg-white/20 px-4 py-2 rounded-lg">
              <Thermometer className="w-5 h-5 text-white" />
              <span className="text-white font-semibold">Thermodynamic Analysis</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                {temperatureUnits.map((unit) => (
                  <button
                    ...
                    className={`py-2 rounded-lg transition-all ${tempUnit === unit.symbol ?
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-4 py-3 border-2 border-orange-200 rounded-lg focus:border-orange-500 focus:ring-2 focus:ring-orange-200 focus:outline-none"
                      placeholder="e.g., 0.1"
                    />
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    />
                    <div className="absolute right-3 top-3 text-gray-500">{tempUnit}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-4 border border-orange-200">
                ...
                >
                  <span className="font-semibold text-orange-700">Show Formula</span>
                  <Zap className={`w-5 h-5 text-orange-600 transform ${showFormula ? 'rotate-180' : ''}`} />
                </button>
                
                {showFormula && (
                  <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200">
                    <h4 className="font-bold text-gray-800 mb-2">Van't Hoff Equation</h4>
                    <div className="text-center p-3 bg-gray-100 rounded-lg font-mono text-sm">
                      ΔH = 2.303 × R × (logS₂ - logS₁) × (T₁ × T₂) / (T₂ - T₁)
                    </div>
                    <div className="mt-3 space-y-2 text-sm">
                      <p><span className="font-semibold">ΔH:</span> Enthalpy of solution (J/mol)</p>
                      <p><span className="font-semibold">R:</span> Gas constant = 8.314 J/(mol·K)</p>
                      <p><span className="font-semibold">S₁, S₂:</span> Solubilities at temperatures T₁ and T₂</p>
                      <p><span className="font-semibold">T₁, T₂:</span> Absolute temperatures in Kelvin</p>
                      <p className="text-xs text-gray-600 mt-2">
                        Note: Positive ΔH indicates endothermic process, negative indicates exothermic
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4">
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-orange-600 to-red-500 hover:from-orange-700 hover:to-red-600 text-white font-semibold py-4 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl"
                >
                  Calculate ΔH
                </button>
                <button
          ...
          <div className="space-y-6">
            {/* Result Card */}
            <div className="bg-gradient-to-br from-orange-600 to-red-500 rounded-2xl shadow-xl p-6 md:p-8 text-white">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <div className="text-xs text-gray-500">T₁ in Kelvin</div>
                      <div className="font-semibold">
                        {tempUnit === 'C' ? (parseFloat(temp1) + 273.15).toFixed(2) : temp1} K
                      </div>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <div className="text-xs text-gray-500">T₂ in Kelvin</div>
                      <div className="font-semibold">
                        {tempUnit === 'C' ? (parseFloat(temp2) + 273.15).toFixed(2) : temp2} K
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                    <div className="text-center font-mono text-sm">
                      ΔH = 2.303 × 8.314 × (log({solubility2}) - log({solubility1})) × 
                      ({(tempUnit === 'C' ? parseFloat(temp1) + 273.15 : temp1)} × 
                      {(tempUnit === 'C' ? parseFloat(temp2) + 273.15 : temp2)}) / 
                      ({(tempUnit === 'C' ? parseFloat(temp2) + 273.15 : temp2)} - 
                      {(tempUnit === 'C' ? parseFloat(temp1) + 273.15 : temp1)})
                    </div>
                  </div>
            ...
            <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-2xl shadow-lg p-6 border border-orange-200">
              <h3 className="text-lg font-bold text-gray-800 mb-4">Common ΔH Values</h3>
              <div className="space-y-3">
                {[
                  ...
                  <div key={index} className="flex items-center justify-between py-2 border-b border-orange-100">
                    <div>
                      <div className="font-medium text-gray-800">{item.substance}</div>
                      <div className="text-xs text-gray-600">{item.type}</div>
                    </div>
                    <div className={`font-bold ${item.enthalpy.startsWith('-') ? 'text-red-600' : 'text-blue-600'}`}>
