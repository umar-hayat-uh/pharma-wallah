...
export default function MCQCard({
  ...
  return (
    <div className="bg-white/30 backdrop-blur-sm rounded-xl border border-white/50 p-6 shadow-md hover:shadow-lg transition-all duration-300">
      ...
      {showResults && mcq.explanation && (
        <div className="mt-4 p-4 bg-blue-50/80 backdrop-blur-sm rounded-lg border border-blue-200 text-sm text-gray-700">
          <span className="font-semibold text-blue-700">Explanation:</span> {mcq.explanation}
        </div>
      )}

      {/* Reference (optional, shown only after submission) */}
      {showResults && (mcq as any).reference && (
        <div className="mt-2 text-xs text-gray-400 italic border-t border-white/20 pt-2">
          Reference: {(mcq as any).reference}
        </div>
      )}
    </div>
  );
