...
// ── Dashboard integration: tracks quiz attempts + activity ──
...
export default function MCQBankQuizPage({ params }: PageProps) {
  ...
  const questions = useMemo(() => {
    if (!bank) return [];
    return activeUnit === "All" ? bank.questions : bank.questions.filter(q => q.unit === activeUnit);
  }, [bank, activeUnit]);
  ...
  useEffect(() => { setAnswers({}); setSubmitted(false); setExpanded(new Set()); setQuizStart(Date.now()); }, [activeUnit]);
  ...
  // Time tracking
  useEffect(() => { const cleanup = trackTimeOnUnmount(); return cleanup; }, [subjectSlug]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    // ── Track quiz result to dashboard ──
    const correct  = questions.filter(q => answers[q.id] === q.correctAnswer).length;
    const timeMins = Math.max(1, Math.round((Date.now() - quizStart) / 60000));
    trackQuiz({
      quizId:       `${subjectSlug}-${activeUnit}-${Date.now()}`,
    ...
    trackActivity({
      type:  "quiz",
      label: `Completed ${subData?.name ?? subjectSlug} MCQs — ${correct}/${questions.length} correct`,
      href:  `/mcqs-bank/${semesterSlug}/${subjectSlug}`,
    });
  }, [submitted, questions, answers, quizStart, subjectSlug, subData, activeUnit, semesterSlug]);
  ...
  if (!bank) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="text-center max-w-sm">
        <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${semGrad} flex items-center justify-center mx-auto mb-4 shadow-lg`}><ClipboardList className="w-8 h-8 text-white" /></div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Link href="/mcqs-bank" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border-2 border-gray-200 text-gray-600 font-extrabold text-sm hover:border-blue-400 transition-all">
  ...
  return (
    <section className="min-h-screen bg-gray-50 relative overflow-x-hidden">
      ...
      <div className={`relative bg-gradient-to-r ${semGrad} overflow-hidden`}>
        ...
        <div className="absolute right-10 bottom-4 opacity-[0.08] pointer-events-none hidden sm:block"><ClipboardList size={120} className="text-white" /></div>
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          ...
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight leading-tight mb-1">{subData.name}</h1>
          <p className="text-white/80 text-sm sm:text-base max-w-xl mb-6">
            {questions.length} question{questions.length !== 1 ? "s" : ""}{activeUnit !== "All" && ` · ${activeUnit}`}{!submitted && ` · ${answeredCount} answered`}
          </p>
          {/* Dashboard link for signed-in users */}
          {isSignedIn && (
            <Link href="/dashboard"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/20 border border-white/30 text-white text-xs font-semibold mb-6 hover:bg-white/30 transition-colors">
              <LayoutDashboard className="w-3.5 h-3.5" /> Track progress in Dashboard
            </Link>
          )}
          {submitted && stats ? (
            <div className="flex flex-wrap gap-3 sm:gap-6">
              {[{ n: `${stats.correct}/${stats.total}`, l: "Correct", col: "text-green-200" }, { n: String(stats.wrong), l: "Wrong", col: "text-red-200" }, { n: String(stats.skipped), l: "Skipped", col: "text-yellow-200" }, { n: `${stats.pct}%`, l: "Score", col: "text-white" }].map(({ n, l, col }) => (
                <div key={l} className="text-center"><div className={`text-2xl sm:text-3xl font-extrabold leading-none ${col}`}>{n}</div><div className="text-xs text-white/70 mt-0.5">{l}</div></div>
              ))}
            </div>
          ) : (
            <div className="flex flex-wrap gap-6">
              {[{ n: questions.length, l: "Questions" }, { n: answeredCount, l: "Answered" }, { n: unansweredCount, l: "Remaining" }].map(({ n, l }) => (
                <div key={l} className="text-center"><div className="text-2xl sm:text-3xl font-extrabold text-white leading-none">{n}</div><div className="text-xs text-white/70 mt-0.5">{l}</div></div>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-10">
            ...
            <div className="flex gap-2 min-w-max">
              ...
              {units.map(u => (
                <button key={u} onClick={() => setActiveUnit(u)}
                  className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl text-xs font-extrabold whitespace-nowrap transition-all duration-200 ${activeUnit === u ? `bg-gradient-to-r ${semGrad} text-white shadow-md` : "bg-white border border-gray-200 text-gray-500 hover:border-blue-300 hover:text-blue-600"}`}>
            ...
            <div className={`mb-6 relative rounded-2xl border-2 ${grade.bg} overflow-hidden p-4 sm:p-6`}>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-600 text-sm mt-0.5">You scored <strong>{stats.correct}</strong> out of <strong>{stats.total}</strong> ({stats.pct}%).{stats.skipped > 0 && <span className="text-amber-600 ml-1">{stats.skipped} unanswered.</span>}</p>
        ...
        <div className="space-y-4 sm:space-y-5">
          {questions.map((q, qIdx) => {
            ...
            const cardBorder = !submitted ? "border-gray-200" : isCorrect ? "border-green-300" : isWrong ? "border-red-300" : "border-amber-300";
            const cardBg     = !submitted ? "bg-white" : isCorrect ? "bg-green-50/40" : isWrong ? "bg-red-50/30" : "bg-amber-50/30";
              ...
              <div key={q.id} className={`relative rounded-2xl border-2 ${cardBorder} ${cardBg} overflow-hidden shadow-sm transition-all duration-300`}>
                ...
                <div className="p-4 sm:p-6">
                  <div className="flex items-start gap-3 mb-4">
                    <div className={`shrink-0 w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center text-xs font-extrabold ${isCorrect ? "bg-green-100 text-green-700 border border-green-200" : isWrong ? "bg-red-100 text-red-700 border border-red-200" : isSkipped ? "bg-amber-100 text-amber-700 border border-amber-200" : `bg-gradient-to-br ${semGrad} text-white`}`}>
                      {isCorrect ? <CheckCircle className="w-4 h-4" /> : isWrong ? <XCircle className="w-4 h-4" /> : isSkipped ? <AlertTriangle className="w-3.5 h-3.5" /> : qIdx + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    {q.options.map(opt => {
                      ...
                      return (
                        <button key={opt.id} onClick={() => handleSelect(q.id, opt.id)} disabled={submitted}
                          ...
                          <span className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-extrabold border ${!submitted && isSel ? "bg-blue-600 text-white border-blue-600" : submitted && isRight ? "bg-green-500 text-white border-green-500" : submitted && isSel ? "bg-red-500 text-white border-red-500" : "bg-gray-100 text-gray-500 border-gray-200"}`}>
                      ...
                      {showExp && (
                        <div className="mt-3 rounded-xl bg-indigo-50 border border-indigo-100 p-3 sm:p-4 space-y-2.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-xs font-extrabold px-2 py-0.5 rounded-lg bg-green-100 text-green-700 border border-green-200">{q.correctAnswer} — {q.options.find(o => o.id === q.correctAnswer)?.text}</span>
                          </div>
                          <div>
        ...
        <div className="mt-8 flex flex-col items-center gap-3">
          {!submitted ? (
            <>
              {unansweredCount > 0 && answeredCount > 0 && (
                <p className="text-xs text-amber-600 font-semibold bg-amber-50 border border-amber-200 px-3 py-2 rounded-xl flex items-center gap-2">
                  <AlertTriangle className="w-3.5 h-3.5 shrink-0" />{unansweredCount} question{unansweredCount > 1 ? "s" : ""} unanswered — you can still submit
                </p>
              )}
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center">
              ...
              {isSignedIn && (
                <Link href="/dashboard"
                  className="inline-flex items-center gap-2 px-6 py-3.5 rounded-2xl border-2 border-blue-200 text-blue-700 font-extrabold text-sm hover:bg-blue-50 transition-all">
                  <LayoutDashboard className="w-4 h-4" /> My Dashboard
                </Link>
              )}
              {subData.href && (
                <Link href={subData.href} className="inline-flex items-center gap-2 px-6 py-3.5 rounded-2xl border-2 border-gray-200 text-gray-600 font-extrabold text-sm hover:border-blue-400 hover:text-blue-600 transition-all">
                  <BookOpen className="w-4 h-4" /> Study Notes
                </Link>
              )}
              <Link href={`/mcqs-bank/${semesterSlug}`} className="inline-flex items-center gap-2 px-6 py-3.5 rounded-2xl border-2 border-gray-200 text-gray-600 font-extrabold text-sm hover:border-blue-400 hover:text-blue-600 transition-all">
