...
import {
  Trophy, HelpCircle, Play, ArrowLeft, Loader2, AlertCircle, ArrowUp,
} from "lucide-react";
...
const GAMES: Record<GameType, GameInfo> = {
  ...
  flashcard: {
    ...
    icon: "⚡",
    description: "Flip the cards and recall key concepts faster than your opponents.",
...
function TournamentGamesInner() {
  ...
  useEffect(() => { validateCode(); }, [validateCode]);

  const includedGames: GameType[] = codeData?.games_included?.filter(
    (g): g is GameType => GAMES.hasOwnProperty(g)
  ) || [];
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {includedGames.map((game) => {
                ...
                return (
                  <div
                    key={game}
                    className={`bg-white border-2 ${info.colorClass} rounded-2xl shadow-sm p-6 flex flex-col justify-between hover:shadow-md transition-all hover:-translate-y-1`}
                  >
                    <div>
