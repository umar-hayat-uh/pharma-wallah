"use client";
// app/spotting/pathology/peptic-ulcer/page.tsx
...
const GRAD = "from-orange-500 to-amber-400";
const TITLE = "Peptic Ulcer";
...
const DATA: {
  ...
  definition: "Peptic ulcer is a full-thickness mucosal defect that penetrates through the muscularis mucosae, occurring in areas exposed to acid-pepsin digestion. Classic sites are the gastric antrum and the first part of the duodenum (duodenal ulcer). H. pylori and NSAIDs are the two principal aetiological factors.",
  generalFeatures: [
  ...
  pathophysiology: "Imbalance between aggressive factors (acid, pepsin, H. pylori, NSAIDs) and defensive factors (mucus, bicarbonate, prostaglandins, epithelial renewal). NSAIDs inhibit COX-1, reducing mucoprotective prostaglandin synthesis. H. pylori undermines the mucus layer and impairs bicarbonate secretion. Acid contact with exposed lamina propria causes progressive tissue necrosis.",
  etiology: [
  ...
  points: [
    "Fibrinoid necrosis at the surface of ulcer crater",
    "Granulation tissue beneath necrotic zone",
    "Fibrosis and chronic inflammation at base",
    "Thrombosed or eroded vessels in the ulcer floor",
    "Intestinal metaplasia at ulcer edge (in stomach)",
  ],
  images: [
    "/images/spotting/pathology/peptic-ulcer-sample.jpg",
    "/images/spotting/pathology/peptic-ulcer-high.jpg",
  ],
  prev: { id: "gastritis", title: "Gastritis" },
  next: { id: "tb-granuloma", title: "TB Granuloma" },
};
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {ALL_LESSONS.map((l, i) => (
            <Link key={l.id} href={`/spotting/pathology/${l.id}`} onClick={onClose}
              className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition group ${l.id === currentId ? "bg-orange-50" : ""}`}>
              ...
              <div className="min-w-0">
                <p className={`text-sm font-bold truncate ${l.id === currentId ? "text-orange-700" : "text-gray-800"}`}>{l.title}</p>
                <p className="text-[10px] text-gray-400">{l.category}</p>
              </div>
              {l.id === currentId && <ChevronRight className="w-3.5 h-3.5 text-orange-400 ml-auto shrink-0" />}
            </Link>
          ))}
        </div>
...
// ─── Main page component ─────────────────────────────────────────────────────
export default function PathologyPepticUlcerPage() {
  ...
  useEffect(() => {
    trackSpotting({
      lessonId: "peptic-ulcer",   // change this per page
      category: "pathology",            // "pathology" | "histology" | "powder-microscopy"
      completed: true,
    });
    trackActivity({
      type: "spotting",
      label: "Opened: Peptic Ulcer", // change label accordingly
    ...
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5 pt-[2.625rem]">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            ...
            <span className="text-orange-700 font-semibold truncate">{TITLE}</span>
          </div>
        ...
        <div className="flex border-t border-gray-100">
          {DATA.prev ? (
            <Link href={`/spotting/pathology/${DATA.prev.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-orange-50 hover:text-orange-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{DATA.prev.title}</span>
            </Link>
          ) : <div className="flex-1" />}
          {DATA.next && (
            <Link href={`/spotting/pathology/${DATA.next.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-orange-50 hover:text-orange-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 pt-4 sm:pt-0 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          <Link href="/spotting" className="hover:text-orange-600 transition-colors">Spotting Centre</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <Link href="/spotting/pathology/lessons" className="hover:text-orange-600 transition-colors">Pathology Lessons</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <span className="text-orange-700 font-semibold">{TITLE}</span>
        </nav>
                  ...
                  {ALL_LESSONS.map(l => (
                    <li key={l.id}>
                      <Link href={`/spotting/pathology/${l.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${l.id === "peptic-ulcer"
                        ...
                        <ChevronRight size={11} className={`shrink-0 transition-opacity ${l.id === "peptic-ulcer" ? "opacity-100" : "opacity-0 group-hover:opacity-60"}`} />
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex-1 min-w-0 w-full space-y-5">
              ...
              <div className="p-4 sm:p-6">
                ...
                <ul className="space-y-2">
                  {DATA.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-3 p-2 rounded-lg bg-orange-50/60 border border-orange-100">
            ...
            <div className={`relative rounded-2xl bg-gradient-to-r ${GRAD} overflow-hidden p-5 sm:p-7`}>
              ...
              <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <Link href="/spotting/pathology/test"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-orange-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all shrink-0">
