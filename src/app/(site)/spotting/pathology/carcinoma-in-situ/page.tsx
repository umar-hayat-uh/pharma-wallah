...
const DATA: {
  ...
  generalFeatures: [
    "SMILE: Stratified Mucin-secreting Intraepithelial Lesion — endocervical crypt involvement",
    "Polyhedral columnar cells with eosinophilic to mucinous cytoplasm",
    "No clear stratification — cells are undifferentiated throughout full thickness",
    "Increased mitotic activity at all levels of the epithelium",
    "Abnormal cells filling the endocervical crypts without invasion",
    "Intact basement membrane — no stromal breach (in situ by definition)",
  ],
  sites: [
    "Squamocolumnar junction (transformation zone) of the cervix",
    "Endocervical crypts involved in CIS/SMILE",
    "May extend into vaginal fornices (VAIN) or endocervical canal",
  ],
  ...
  etiology: [
    "HPV 16 (most oncogenic): associated with squamous CIS",
    "HPV 18: more associated with adenocarcinoma in situ",
    "Risk factors: early sexual activity, multiple partners, smoking, immunosuppression",
    "Previous CIN 1/2 progression",
    "HIV infection: greatly elevated risk",
  ],
  clinicalFeatures: [
    "Usually asymptomatic — detected on cervical screening (Pap smear / liquid-based cytology)",
    "Post-coital bleeding if associated with erosion",
    "Colposcopy: acetowhite areas, punctation, mosaicism at transformation zone",
    "High-grade changes on cervical smear (HSIL / CIN 3 cytology)",
    "Progression to invasive cancer if untreated",
  ],
  diagnosis: [
    "Cervical smear (LBC): HSIL cells",
    "HPV genotyping: high-risk HPV 16/18",
    "Colposcopy and targeted biopsy: gold standard",
    "Histopathology: full-thickness dysplasia, SMILE, intact BM",
    "Large loop excision of the transformation zone (LLETZ): diagnostic and therapeutic",
  ],
  treatment: [
    "LLETZ (LEEP): electrosurgical loop excision — first-line treatment for CIN 3 / CIS",
    "Cone biopsy: cold knife cone — for glandular lesions or depth needed",
    "Cryotherapy / laser ablation: for carefully selected CIN 2–3",
    "Hysterectomy: for recurrent CIS or completed family",
    "HPV vaccination (Gardasil-9): primary prevention against HPV 16/18",
  ],
  points: [
    "Full-thickness loss of epithelial maturation (CIN 3)",
    "Intact basement membrane – no stromal invasion",
    "Increased mitotic figures, including at upper levels",
    "Nuclear atypia (pleomorphism, hyperchromasia)",
    "May involve endocervical crypts (SMILE)",
  ],
  images: [
    "/images/spotting/pathology/carcinoma-in-situ-sample.JPG",
    "/images/spotting/pathology/carcinoma-in-situ-high.jpg",
  ],
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {ALL_LESSONS.map((l, i) => (
            <Link key={l.id} href={`/spotting/pathology/${l.id}`} onClick={onClose}
              className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition group ${l.id === currentId ? "bg-fuchsia-50" : ""}`}>
              ...
              <div className="min-w-0">
                <p className={`text-sm font-bold truncate ${l.id === currentId ? "text-fuchsia-700" : "text-gray-800"}`}>{l.title}</p>
                <p className="text-[10px] text-gray-400">{l.category}</p>
              </div>
              {l.id === currentId && <ChevronRight className="w-3.5 h-3.5 text-fuchsia-400 ml-auto shrink-0" />}
            </Link>
          ))}
        </div>
...
// ─── Main page component ─────────────────────────────────────────────────────
export default function CarcinomaInSituPage() {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [lightbox, setLightbox] = useState<string | null>(null);
  const lessonIdx = ALL_LESSONS.findIndex(l => l.id === "carcinoma-in-situ");
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {BG_ICONS.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-fuchsia-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      {/* Navigation drawer */}
      <LessonNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} currentId="carcinoma-in-situ" />
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href="/spotting/pathology/lessons" className="hover:text-fuchsia-600 shrink-0 font-medium">Pathology</Link>
            <ChevronRight size={11} className="mx-0.5 text-gray-300 shrink-0" />
            <span className="text-fuchsia-700 font-semibold truncate">{TITLE}</span>
          </div>
        ...
        <div className="flex border-t border-gray-100">
          {DATA.prev ? (
            <Link href={`/spotting/pathology/${DATA.prev.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-fuchsia-50 hover:text-fuchsia-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{DATA.prev.title}</span>
            </Link>
          ) : <div className="flex-1" />}
          {DATA.next && (
            <Link href={`/spotting/pathology/${DATA.next.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-fuchsia-50 hover:text-fuchsia-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          <Link href="/spotting" className="hover:text-fuchsia-600 transition-colors">Spotting Centre</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <Link href="/spotting/pathology/lessons" className="hover:text-fuchsia-600 transition-colors">Pathology Lessons</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <span className="text-fuchsia-700 font-semibold">{TITLE}</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-8 items-start w-full">
                  ...
                  {ALL_LESSONS.map(l => (
                    <li key={l.id}>
                      <Link href={`/spotting/pathology/${l.id}`}
                        ...
                        <ChevronRight size={11} className={`shrink-0 transition-opacity ${l.id === "carcinoma-in-situ" ? "opacity-100" : "opacity-0 group-hover:opacity-60"}`} />
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex-1 min-w-0 w-full space-y-5">
            ...
            {/* Points of Recognition (changed from Identification) */}
              ...
              <div className="p-4 sm:p-6">
                ...
                <ul className="space-y-2">
                  {DATA.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-3 p-2 rounded-lg bg-fuchsia-50/60 border border-fuchsia-100">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {DATA.prev ? (
                <Link href={`/spotting/pathology/${DATA.prev.id}`}
                  ...
                  <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${GRAD}`} />
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-fuchsia-50 border border-fuchsia-100 flex items-center justify-center flex-shrink-0">
                    <ChevronLeft size={15} className="text-fuchsia-600" />
                  </div>
                  <div className="min-w-0">
              ...
              {DATA.next ? (
                <Link href={`/spotting/pathology/${DATA.next.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-fuchsia-50 border border-fuchsia-100 flex items-center justify-center flex-shrink-0">
                    <ChevronRight size={15} className="text-fuchsia-600" />
                  </div>
                </Link>
              ) : <div />}
            </div>
            ...
            <div className={`relative rounded-2xl bg-gradient-to-r ${GRAD} overflow-hidden p-5 sm:p-7`}>
              ...
              <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <Link href="/spotting/pathology/test"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-fuchsia-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all shrink-0">
