...
const EMOJI = "🫁"; // choose appropriate emoji
...
const DATA: {
  ...
  generalFeatures: [
    "Rokitansky–Aschoff sinuses (RAS): epithelium-lined crypts herniating through the muscularis",
    "Mononuclear inflammatory infiltrate (lymphocytes, plasma cells) in lamina propria",
    "Sub-epithelial and sub-serosal fibrosis causing gallbladder shrinkage",
    "Hypertrophy of the muscularis propria",
    "Cholesterol clefts or calcium deposits may be present",
    "Mucosal folds may be flattened or atrophied",
  ],
  sites: [
    "Gallbladder — body and fundus most commonly affected",
    "Associated with gallstones in >90% of cases",
    "Porcelain gallbladder: dystrophic calcification of the wall (rare)",
  ],
  ...
  etiology: [
    "Cholelithiasis (gallstones) — present in >90%",
    "Acalculous chronic cholecystitis — rare, secondary to ischaemia or infection",
    "Risk factors: female, fat, forty, fertile (5 Fs)",
    "Salmonella typhi chronic carrier state can cause cholecystitis",
  ],
  clinicalFeatures: [
    "Recurrent right upper quadrant or epigastric pain (biliary colic)",
    "Nausea and fat intolerance",
    "Pain precipitated by fatty meals (cholecystokinin release)",
    "Murphy's sign positive on deep palpation",
    "Jaundice if common bile duct obstructed (Mirizzi syndrome)",
  ],
  diagnosis: [
    "Ultrasound: gallstones, thickened gallbladder wall (>3 mm), pericholecystic fluid",
    "HIDA scan: reduced gallbladder ejection fraction (<35%)",
    "CT/MRCP: complications and anatomy",
    "Histopathology post-cholecystectomy: confirms chronic inflammation and RAS",
  ],
  treatment: [
    "Elective laparoscopic cholecystectomy — definitive treatment",
    "Low-fat diet for symptomatic relief pre-operatively",
    "ERCP with stone extraction if choledocholithiasis present",
    "Ursodeoxycholic acid (UDCA) for dissolution in non-surgical candidates",
  ],
  points: [
    "Rokitansky–Aschoff sinuses (epithelial herniations through muscularis)",
    "Chronic inflammatory infiltrate (lymphocytes, plasma cells)",
    "Fibrosis and thickening of gallbladder wall",
    "Associated gallstones in most cases",
  ],
  images: [
    "/images/spotting/pathology/chronic-cholecystitis-sample.avif",
    "/images/spotting/pathology/chronic-cholecystitis-high.jpg",
  ],
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function ChronicCholecystitisPage() {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [lightbox, setLightbox] = useState<string | null>(null);
  const lessonIdx = ALL_LESSONS.findIndex(l => l.id === "chronic-cholecystitis");
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      <LessonNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} currentId="chronic-cholecystitis" />
