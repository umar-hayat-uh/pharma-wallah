"use client";
// app/spotting/pathology/lessons/adenocarcinoma/page.tsx
...
const GRAD = "from-emerald-600 to-teal-400";
const TITLE = "Adenocarcinoma";
const CAT = "Malignant Tumour";
const EMOJI = "🦠";
const VIDEO_URL = "https://youtu.be/Sh65aXndqXk";
const VIDEO_TITLE = "Adenocarcinoma — Histopathology & Molecular Pathology";
const PO_URL = "https://www.pathologyoutlines.com/topic/colontumoradenocarcinoma.html";
...
const DATA: {
  ...
  definition: "Adenocarcinoma is a malignant epithelial tumour showing glandular differentiation. Histologically it features irregular malignant glandular structures lined by pleomorphic columnar cells, abundant intraluminal mucin, infiltration through the muscularis mucosae, desmoplastic stromal reaction, and lymphovascular invasion.",
  generalFeatures: [
    "Irregular, atypical glandular structures — architectural distortion",
    "Abundant intraluminal mucin (mucinous variant: >50% mucin pools)",
    "Cancer cells infiltrating through the muscularis mucosae into submucosa",
    "Lymphocytic and plasma cell infiltration in the stroma",
    "Desmoplastic stromal reaction around tumour glands",
    "Metastasis to regional lymph nodes and distant organs",
  ],
  sites: [
    "Colorectum: most common adenocarcinoma site in Western countries",
    "Stomach: intestinal type (H. pylori–driven) and diffuse type (signet-ring cells)",
    "Lung: peripheral, arising from pneumocytes — most common lung cancer type",
    "Pancreas: ductal adenocarcinoma — head of pancreas (75%)",
    "Breast: invasive ductal carcinoma — most common breast malignancy",
    "Prostate, endometrium, ovary, cervix (endocervical type)",
  ],
  pathophysiology: "Adenocarcinoma follows the adenoma–carcinoma sequence (colon) or Barrett's metaplasia–dysplasia–adenocarcinoma sequence (oesophagus). Driver mutations include KRAS, TP53, SMAD4 (colon); CDH1, RUNX3 (gastric); EGFR, ALK, ROS1 (lung). Invasion is mediated by MMPs and E-cadherin loss (epithelial–mesenchymal transition).",
  etiology: [
    "Colonic: adenoma–carcinoma sequence; HNPCC (Lynch syndrome), FAP",
    "Gastric: H. pylori → chronic gastritis → intestinal metaplasia → carcinoma",
    "Lung: smoking (though adenocarcinoma is the commonest type in non-smokers)",
    "Pancreatic: smoking, chronic pancreatitis, hereditary pancreatitis",
  ],
  clinicalFeatures: [
    "Colorectal: change in bowel habit, rectal bleeding, weight loss, mass per rectum",
    "Gastric: dysphagia, weight loss, early satiety, Virchow's node",
    "Lung: haemoptysis, pleural effusion, clubbing",
    "Pancreatic: painless obstructive jaundice, Courvoisier's sign, steatorrhoea",
  ],
  diagnosis: [
    "Endoscopy and biopsy: colonoscopy for colorectal, OGD for gastric",
    "CT/PET-CT: staging and metastasis assessment",
    "Histopathology: glandular architecture, invasion, desmoplasia",
    "Molecular profiling: KRAS, EGFR, ALK, HER2, MSI/MMR status",
  ],
  treatment: [
    "Surgical resection: hemicolectomy, gastrectomy, pneumonectomy, Whipple's",
    "Adjuvant chemotherapy: FOLFOX (colorectal), FLOT (gastric), carboplatin-paclitaxel (lung)",
    "Targeted therapy: bevacizumab (anti-VEGF), cetuximab (anti-EGFR), osimertinib (EGFR-mutant lung)",
    "Immunotherapy: pembrolizumab for MSI-high or PD-L1 positive tumours",
  ],
  points: [
    "Malignant glandular structures infiltrating stroma",
    "Pleomorphic columnar cells with nuclear hyperchromasia",
    "Intraluminal mucin and necrotic debris",
    "Desmoplastic stromal reaction",
    "Lymphovascular invasion (in advanced cases)",
  ],
  images: [
    "/images/spotting/pathology/adenocarcinoma-sample.png",
    "/images/spotting/pathology/adenocarcinoma-high.jpg",
  ],
  prev: { id: "hodgkin-lymphoma", title: "Hodgkin's Disease" },
  next: { id: "fatty-liver", title: "Fatty Liver" },
};
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {ALL_LESSONS.map((l, i) => (
            <Link key={l.id} href={`/spotting/pathology/${l.id}`} onClick={onClose}
              className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition group ${l.id === currentId ? "bg-emerald-50" : ""}`}>
              ...
              <div className="min-w-0">
                <p className={`text-sm font-bold truncate ${l.id === currentId ? "text-emerald-700" : "text-gray-800"}`}>{l.title}</p>
                <p className="text-[10px] text-gray-400">{l.category}</p>
              </div>
              {l.id === currentId && <ChevronRight className="w-3.5 h-3.5 text-emerald-400 ml-auto shrink-0" />}
            </Link>
          ))}
        </div>
...
// ─── Main page component ─────────────────────────────────────────────────────
export default function AdenocarcinomaPage() {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [lightbox, setLightbox] = useState<string | null>(null);
  const lessonIdx = ALL_LESSONS.findIndex(l => l.id === "adenocarcinoma");
  ...
  useEffect(() => {
    trackSpotting({
      lessonId: "adenocarcinoma",   // change this per page
      category: "pathology",            // "pathology" | "histology" | "powder-microscopy"
      completed: true,
    });
    trackActivity({
      type: "spotting",
      label: "Opened: Adenocarcinoma", // change label accordingly
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {BG_ICONS.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-emerald-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      {/* Navigation drawer */}
      <LessonNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} currentId="adenocarcinoma" />
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5 pt-[2.625rem]">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href="/spotting/pathology/lessons" className="hover:text-emerald-600 shrink-0 font-medium">Pathology</Link>
            <ChevronRight size={11} className="mx-0.5 text-gray-300 shrink-0" />
            <span className="text-emerald-700 font-semibold truncate">{TITLE}</span>
          </div>
        ...
        <div className="flex border-t border-gray-100">
          {DATA.prev ? (
            <Link href={`/spotting/pathology/${DATA.prev.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{DATA.prev.title}</span>
            </Link>
          ) : <div className="flex-1" />}
          {DATA.next && (
            <Link href={`/spotting/pathology/${DATA.next.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 pt-4 sm:pt-0 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          <Link href="/spotting" className="hover:text-emerald-600 transition-colors">Spotting Centre</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <Link href="/spotting/pathology/lessons" className="hover:text-emerald-600 transition-colors">Pathology Lessons</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <span className="text-emerald-700 font-semibold">{TITLE}</span>
        </nav>
                  ...
                  {ALL_LESSONS.map(l => (
                    <li key={l.id}>
                      <Link href={`/spotting/pathology/${l.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${l.id === "adenocarcinoma"
                        ...
                        <ChevronRight size={11} className={`shrink-0 transition-opacity ${l.id === "adenocarcinoma" ? "opacity-100" : "opacity-0 group-hover:opacity-60"}`} />
                      </Link>
                    </li>
                  ))}
                </ul>
              ...
              <div className="p-4 sm:p-6">
                ...
                <ul className="space-y-2">
                  {DATA.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-3 p-2 rounded-lg bg-emerald-50/60 border border-emerald-100">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {DATA.prev ? (
                <Link href={`/spotting/pathology/${DATA.prev.id}`}
                  ...
                  <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${GRAD}`} />
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center justify-center flex-shrink-0">
                    <ChevronLeft size={15} className="text-emerald-600" />
                  </div>
                  <div className="min-w-0">
