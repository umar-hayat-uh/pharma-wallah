"use client";
// app/spotting/pathology/lipoma/page.tsx
...
const GRAD = "from-lime-600 to-green-400";
const TITLE = "Lipoma";
const CAT = "Soft Tissue Tumour";
const EMOJI = "🧈";
const VIDEO_URL = "https://youtu.be/Dzdzb3LRLxs";
const VIDEO_TITLE = "Lipoma — Soft Tissue Tumour Pathology";
const PO_URL = "https://www.pathologyoutlines.com/topic/softtissueadiposelipoma.html";
...
const DATA: {
  ...
  definition: "Lipoma is the most common benign soft-tissue tumour in adults, composed of mature adipocytes arranged in lobules separated by delicate fibrovascular septa. It is characterised by clear cytoplasm and peripherally displaced nuclei with no atypia, mitoses, or lipoblasts.",
  generalFeatures: [
    "Lobules of mature adipocytes with clear (lipid-filled) cytoplasm",
    "Peripherally located, small, flattened nuclei — no atypia or hyperchromasia",
    "Delicate fibrovascular septa separating lobules",
    "No lipoblasts (distinguish from well-differentiated liposarcoma)",
    "No increased mitotic activity",
    "May have thick fibrous capsule or blend imperceptibly into surrounding fat",
  ],
  sites: [
    "Subcutaneous tissue of trunk, shoulders, neck, and proximal extremities (most common)",
    "Intramuscular lipoma: deeper, less mobile, higher recurrence",
    "Spinal canal: epidural lipoma — compresses cord",
    "GI tract, breast, parotid (rare visceral sites)",
  ],
  pathophysiology: "Lipomas arise from clonal proliferation of mature adipocytes. Cytogenetic aberrations in chromosomes 12q, 13q, or 6p are common. The HMGA2 gene rearrangement on 12q15 is the most frequent molecular event. Angiolipomas contain a vascular component and are often multiple and painful.",
  etiology: [
    "Sporadic clonal mutations — most common",
    "Multiple hereditary lipomatosis: autosomal dominant",
    "Familial multiple lipomatosis: multiple lipomas often on forearms",
    "MEN1: parathyroid adenoma, pituitary adenoma, and lipomas",
  ],
  clinicalFeatures: [
    "Soft, mobile, painless subcutaneous lump",
    "Slow growing; rarely exceeds 5 cm",
    "Angiolipoma: painful, especially after pressure",
    "Large intramuscular lipoma: firmness, pseudo-malignant feel",
    "No systemic features; malignant transformation is exceedingly rare",
  ],
  diagnosis: [
    "Clinical diagnosis: soft, mobile, painless lesion",
    "Ultrasound: hyperechoic or isoechoic lobulated lesion",
    "MRI: signal characteristics identical to normal fat — most reliable",
    "Excision biopsy and histopathology: definitive diagnosis",
  ],
  treatment: [
    "Observation if asymptomatic and <5 cm",
    "Surgical excision: for cosmetic reasons, pain, or diagnostic doubt",
    "Liposuction: alternative for superficial lipomas",
    "Intramuscular lipomas: wider excision to reduce recurrence",
  ],
  points: [
    "Lobules of mature adipocytes",
    "Peripheral (compressed) nuclei",
    "Thin fibrous capsule",
    "Absence of atypia or lipoblasts",
    "Capillaries often present between adipocytes",
  ],
  images: [
    "/images/spotting/pathology/lipoma-high.jpg",
    "/images/spotting/pathology/lipoma-mid.jpg",
  ],
  prev: { id: "leiomyoma", title: "Leiomyoma" },
  next: { id: "squamous-cell-carcinoma", title: "Squamous Cell Carcinoma" },
};
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {ALL_LESSONS.map((l, i) => (
            <Link key={l.id} href={`/spotting/pathology/${l.id}`} onClick={onClose}
              className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition group ${l.id === currentId ? "bg-lime-50" : ""}`}>
              ...
              <div className="min-w-0">
                <p className={`text-sm font-bold truncate ${l.id === currentId ? "text-lime-700" : "text-gray-800"}`}>{l.title}</p>
                <p className="text-[10px] text-gray-400">{l.category}</p>
              </div>
              {l.id === currentId && <ChevronRight className="w-3.5 h-3.5 text-lime-400 ml-auto shrink-0" />}
            </Link>
          ))}
        </div>
...
// ─── Main page component ─────────────────────────────────────────────────────
export default function PathologyLipomaPage() {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [lightbox, setLightbox] = useState<string | null>(null);
  const lessonIdx = ALL_LESSONS.findIndex(l => l.id === "lipoma");
  ...
  useEffect(() => {
    trackSpotting({
      lessonId: "lipoma",   // change this per page
      category: "pathology",            // "pathology" | "histology" | "powder-microscopy"
      completed: true,
    });
    trackActivity({
      type: "spotting",
      label: "Opened: Lipoma", // change label accordingly
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {BG_ICONS.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-lime-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      {/* Navigation drawer */}
      <LessonNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} currentId="lipoma" />
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5 pt-[2.625rem]">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href="/spotting/pathology/lessons" className="hover:text-lime-600 shrink-0 font-medium">Pathology</Link>
            <ChevronRight size={11} className="mx-0.5 text-gray-300 shrink-0" />
            <span className="text-lime-700 font-semibold truncate">{TITLE}</span>
          </div>
        ...
        <div className="flex border-t border-gray-100">
          {DATA.prev ? (
            <Link href={`/spotting/pathology/${DATA.prev.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-lime-50 hover:text-lime-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{DATA.prev.title}</span>
            </Link>
          ) : <div className="flex-1" />}
          {DATA.next && (
            <Link href={`/spotting/pathology/${DATA.next.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-lime-50 hover:text-lime-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 pt-4 sm:pt-0 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          <Link href="/spotting" className="hover:text-lime-600 transition-colors">Spotting Centre</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <Link href="/spotting/pathology/lessons" className="hover:text-lime-600 transition-colors">Pathology Lessons</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <span className="text-lime-700 font-semibold">{TITLE}</span>
        </nav>
                  ...
                  {ALL_LESSONS.map(l => (
                    <li key={l.id}>
                      <Link href={`/spotting/pathology/${l.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${l.id === "lipoma"
                        ...
                        <ChevronRight size={11} className={`shrink-0 transition-opacity ${l.id === "lipoma" ? "opacity-100" : "opacity-0 group-hover:opacity-60"}`} />
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex-1 min-w-0 w-full space-y-5">
              ...
              <div className="p-4 sm:p-6">
                ...
                <ul className="space-y-2">
                  {DATA.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-3 p-2 rounded-lg bg-lime-50/60 border border-lime-100">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {DATA.prev ? (
                <Link href={`/spotting/pathology/${DATA.prev.id}`}
                  ...
                  <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${GRAD}`} />
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-lime-50 border border-lime-100 flex items-center justify-center flex-shrink-0">
                    <ChevronLeft size={15} className="text-lime-600" />
                  </div>
                  <div className="min-w-0">
              ...
              {DATA.next ? (
                <Link href={`/spotting/pathology/${DATA.next.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-lime-50 border border-lime-100 flex items-center justify-center flex-shrink-0">
                    <ChevronRight size={15} className="text-lime-600" />
                  </div>
                </Link>
              ) : <div />}
            </div>
            ...
            <div className={`relative rounded-2xl bg-gradient-to-r ${GRAD} overflow-hidden p-5 sm:p-7`}>
              ...
              <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <Link href="/spotting/pathology/test"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-lime-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all shrink-0">
