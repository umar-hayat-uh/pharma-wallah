"use client";
// app/spotting/pathology/squamous-cell-carcinoma/page.tsx
...
const TITLE = "Squamous Cell Carcinoma";
const CAT = "Malignant Tumour";
const EMOJI = "🔥";
const VIDEO_URL = "https://youtu.be/jyjc2U4Qmfo?si=h79eS8Q-3hTRN6is";
const VIDEO_TITLE = "Squamous Cell Carcinoma — Histopathology Lecture";
const PO_URL = "https://www.pathologyoutlines.com/topic/skintumornonmelanocyticscc.html";
...
const DATA: {
  ...
  definition: "Squamous cell carcinoma (SCC) is a malignant tumour of squamous epithelial cells, characterised by large polygonal cells with eosinophilic cytoplasm, intercellular desmosomes (prickle cells), keratin pearl formation, and stromal invasion. It arises from stratified squamous epithelium or from squamous metaplasia.",
  generalFeatures: [
    "Large polygonal cells with abundant eosinophilic cytoplasm",
    "Concentric whorls of keratinised cells forming keratin pearls",
    "Desmosomal connections (intercellular bridges) between adjacent cells",
    "Tumour cell infiltration into the underlying stroma, breaching the basement membrane",
    "Lymphocytic infiltrate ± plasma cells in the stroma (host response)",
    "Nuclear pleomorphism, hyperchromasia, and abnormal mitoses",
  ],
  sites: [
    "Skin: sun-exposed areas, particularly head, neck, dorsal hands",
    "Oral cavity: tongue, floor of mouth (smoking + alcohol)",
    "Oesophagus: middle and lower third",
    "Lung: hilar, central, arising from bronchial epithelium (smoking)",
    "Cervix: squamocolumnar junction (HPV 16/18)",
    "Penis, vulva, anal canal",
  ],
  pathophysiology: "UV radiation, tobacco carcinogens, or HPV oncoproteins (E6/E7) disrupt p53 and Rb tumour-suppressor pathways, leading to uncontrolled proliferation of squamous cells. Cells lose polarity, invade the stroma, and produce dyskeratotic cells. Desmoplastic reaction around tumour nests provides structural support for invasion and metastasis.",
  etiology: [
    "UV radiation (skin SCC): UVB-induced p53 mutation",
    "Smoking and alcohol (oral, oesophageal, lung SCC)",
    "HPV 16 and 18: cervical, oropharyngeal, and anogenital SCC",
    "Chronic irritation: Marjolin's ulcer (SCC arising in chronic wounds/scars)",
    "Immunosuppression: organ transplant recipients — greatly elevated risk",
  ],
  clinicalFeatures: [
    "Skin: indurated, keratotic nodule or ulcer with everted edges",
    "Oral: non-healing ulcer, leukoplakia, erythroplakia",
    "Lung: haemoptysis, recurrent pneumonia, Pancoast syndrome",
    "Cervix: post-coital bleeding, foul-smelling discharge",
    "Metastasis: regional lymph nodes (hard, non-tender), distant organs",
  ],
  diagnosis: [
    "Biopsy and histopathology: gold standard",
    "Histology: keratin pearls, desmosomes, stromal invasion",
    "HPV testing (cervical): PCR-based genotyping",
    "Staging CT/PET-CT: lymph node and distant metastasis assessment",
  ],
  treatment: [
    "Surgical excision with clear margins: primary treatment for localised SCC",
    "Mohs micrographic surgery: for facial/skin SCC — margin controlled",
    "Radiotherapy: for non-surgical candidates or adjuvant post-excision",
    "Chemoradiation (cisplatin-based): locally advanced disease",
    "Immune checkpoint inhibitors (pembrolizumab): metastatic/recurrent SCC",
  ],
  points: [
    "Keratin pearls (concentric whorls of keratin)",
    "Intercellular bridges (prickles) between cells",
    "Invasion of dermis/submucosa by malignant squamous nests",
    "Nuclear atypia and atypical mitoses",
    "Individual cell keratinisation (dyskeratosis)",
  ],
  images: [
    "/images/spotting/pathology/squamous-cell-carcinoma-sample.jpg",
    "/images/spotting/pathology/squamous-cell-carcinoma.jpg",
  ],
  prev: { id: "lipoma", title: "Lipoma" },
  next: { id: "hodgkin-lymphoma", title: "Hodgkin's Disease" },
};
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {ALL_LESSONS.map((l, i) => (
            <Link key={l.id} href={`/spotting/pathology/${l.id}`} onClick={onClose}
              className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition group ${l.id === currentId ? "bg-red-50" : ""}`}>
              ...
              <div className="min-w-0">
                <p className={`text-sm font-bold truncate ${l.id === currentId ? "text-red-700" : "text-gray-800"}`}>{l.title}</p>
                <p className="text-[10px] text-gray-400">{l.category}</p>
              </div>
              {l.id === currentId && <ChevronRight className="w-3.5 h-3.5 text-red-400 ml-auto shrink-0" />}
            </Link>
          ))}
        </div>
...
// ─── Main page component ─────────────────────────────────────────────────────
export default function PathologySquamousCellCarcinomaPage() {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [lightbox, setLightbox] = useState<string | null>(null);
  const lessonIdx = ALL_LESSONS.findIndex(l => l.id === "squamous-cell-carcinoma");
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {BG_ICONS.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-red-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      {/* Navigation drawer */}
      <LessonNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} currentId="squamous-cell-carcinoma" />
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href="/spotting/pathology/lessons" className="hover:text-red-600 shrink-0 font-medium">Pathology</Link>
            <ChevronRight size={11} className="mx-0.5 text-gray-300 shrink-0" />
            <span className="text-red-700 font-semibold truncate">{TITLE}</span>
          </div>
        ...
        <div className="flex border-t border-gray-100">
          {DATA.prev ? (
            <Link href={`/spotting/pathology/${DATA.prev.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-red-50 hover:text-red-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{DATA.prev.title}</span>
            </Link>
          ) : <div className="flex-1" />}
          {DATA.next && (
            <Link href={`/spotting/pathology/${DATA.next.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-red-50 hover:text-red-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          <Link href="/spotting" className="hover:text-red-600 transition-colors">Spotting Centre</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <Link href="/spotting/pathology/lessons" className="hover:text-red-600 transition-colors">Pathology Lessons</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <span className="text-red-700 font-semibold">{TITLE}</span>
        </nav>
          ...
          <div className="flex-1 min-w-0 w-full space-y-5">
              ...
              <div className="p-4 sm:p-6">
                ...
                <ul className="space-y-2">
                  {DATA.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-3 p-2 rounded-lg bg-red-50/60 border border-red-100">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {DATA.prev ? (
                <Link href={`/spotting/pathology/${DATA.prev.id}`}
                  ...
                  <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${GRAD}`} />
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-red-50 border border-red-100 flex items-center justify-center flex-shrink-0">
                    <ChevronLeft size={15} className="text-red-600" />
                  </div>
                  <div className="min-w-0">
              ...
              {DATA.next ? (
                <Link href={`/spotting/pathology/${DATA.next.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-red-50 border border-red-100 flex items-center justify-center flex-shrink-0">
                    <ChevronRight size={15} className="text-red-600" />
                  </div>
                </Link>
              ) : <div />}
            </div>
            ...
            <div className={`relative rounded-2xl bg-gradient-to-r ${GRAD} overflow-hidden p-5 sm:p-7`}>
              ...
              <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <Link href="/spotting/pathology/test"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-red-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all shrink-0">
