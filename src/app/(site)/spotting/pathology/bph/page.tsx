"use client";
// app/spotting/pathology/lessons/bph/page.tsx
...
const GRAD = "from-sky-600 to-cyan-400";
const TITLE = "Benign Prostatic Hyperplasia";
const CAT = "Urological";
const EMOJI = "🧔";
const VIDEO_URL = "https://youtu.be/v7le4BplQ78?si=fTLoiqb7sZix-eBw";
const VIDEO_TITLE = "BPH — Benign Prostatic Hyperplasia Pathology Lecture";
const PO_URL = "https://www.pathologyoutlines.com/topic/prostatenodhyper.html";
...
const DATA: {
  ...
  definition: "Benign Prostatic Hyperplasia (BPH) is non-neoplastic proliferation of both glandular and stromal elements in the periurethral transitional zone of the prostate, driven by dihydrotestosterone (DHT). Histologically it features enlarged glands with papillary projections lined by double-layer epithelium (inner columnar + outer cuboidal basal cells) and cystic dilation.",
  generalFeatures: [
    "Cystic glandular spaces with papillary projections into the lumen",
    "Double-layer epithelium: inner tall columnar secretory cells + outer cuboidal basal cells",
    "Corpora amylacea: laminated eosinophilic concretions in gland lumens",
    "Increased fibromuscular stromal component",
    "Secondary bladder changes: detrusor hypertrophy, diverticulum",
    "No invasion, no significant atypia — distinguishes from carcinoma",
  ],
  sites: [
    "Transitional (periurethral) zone of the prostate — exclusive site of BPH",
    "Peripheral zone: site of most prostate carcinomas (not BPH)",
    "Enlargement compresses the prostatic urethra causing LUTS",
  ],
  pathophysiology: "Testosterone is converted to DHT by 5α-reductase in stromal cells. DHT binds androgen receptors and promotes epithelial and stromal proliferation. Relative androgen/oestrogen imbalance with ageing amplifies this effect. Mechanical obstruction of the urethra causes LUTS; chronic retention leads to hydronephrosis.",
  etiology: [
    "Ageing: prevalence increases from 8% at 31–40 years to 80% by age 80",
    "Androgens: DHT via 5α-reductase is the key driver",
    "Oestrogen: relative increase with ageing may sensitise stroma",
    "Genetic: strong familial component; multiple susceptibility loci",
  ],
  clinicalFeatures: [
    "Lower urinary tract symptoms (LUTS): hesitancy, poor stream, terminal dribbling",
    "Storage symptoms: frequency, urgency, nocturia",
    "Acute urinary retention: precipitated by constipation, cold, anticholinergics",
    "Digital rectal exam: symmetrically enlarged, smooth, firm, non-tender prostate",
    "Elevated PSA (but lower density than in carcinoma)",
  ],
  diagnosis: [
    "IPSS score: International Prostate Symptom Score — severity assessment",
    "Uroflowmetry: reduced peak flow rate (<10 mL/s)",
    "Post-void residual volume (PVRV): ultrasound guided",
    "PSA: to exclude carcinoma",
    "Cystoscopy / TRUS biopsy if PSA elevated or suspicious features",
  ],
  treatment: [
    "Alpha-1 blockers (tamsulosin, alfuzosin): relax smooth muscle — first line",
    "5α-reductase inhibitors (finasteride, dutasteride): shrink gland — for large glands",
    "Combination therapy: alpha-blocker + 5-ARI for moderate-severe symptoms",
    "TURP (transurethral resection of prostate): gold-standard surgical treatment",
    "Laser prostatectomy (HoLEP), UroLift, Rezum: minimally invasive alternatives",
  ],
  points: [
    "Nodular hyperplasia of glands and stroma in transitional zone",
    "Cystically dilated glands with papillary infoldings",
    "Double-layered epithelium (basal + luminal)",
    "Corpora amylacea in gland lumens",
    "Stromal component: smooth muscle and fibrous tissue",
  ],
  images: [
    "/images/spotting/pathology/bph-sample.jpg",
    "/images/spotting/pathology/bph-high.jpg",
  ],
  prev: { id: "cvc-liver", title: "Chronic Venous Congestion (Liver)" },
  next: { id: "fibroadenoma", title: "Fibroadenoma" },
};
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {ALL_LESSONS.map((l, i) => (
            <Link key={l.id} href={`/spotting/pathology/${l.id}`} onClick={onClose}
              className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition group ${l.id === currentId ? "bg-sky-50" : ""}`}>
              ...
              <div className="min-w-0">
                <p className={`text-sm font-bold truncate ${l.id === currentId ? "text-sky-700" : "text-gray-800"}`}>{l.title}</p>
                <p className="text-[10px] text-gray-400">{l.category}</p>
              </div>
              {l.id === currentId && <ChevronRight className="w-3.5 h-3.5 text-sky-400 ml-auto shrink-0" />}
            </Link>
          ))}
        </div>
...
export default function BphPage() {
  ...
  const lessonIdx = ALL_LESSONS.findIndex(l => l.id === "bph");
  ...
  useEffect(() => {
    trackSpotting({
      lessonId: "bph",   // change this per page
      category: "pathology",            // "pathology" | "histology" | "powder-microscopy"
      completed: true,
    });
    trackActivity({
      type: "spotting",
      label: "Opened: Benign Prostatic Hyperplasia", // change label accordingly
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {BG_ICONS.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-sky-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      {/* Navigation drawer */}
      <LessonNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} currentId="bph" />
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5 pt-[2.625rem]">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href="/spotting/pathology/lessons" className="hover:text-sky-600 shrink-0 font-medium">Pathology</Link>
            <ChevronRight size={11} className="mx-0.5 text-gray-300 shrink-0" />
            <span className="text-sky-700 font-semibold truncate">{TITLE}</span>
          </div>
        ...
        <div className="flex border-t border-gray-100">
          {DATA.prev ? (
            <Link href={`/spotting/pathology/${DATA.prev.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-sky-50 hover:text-sky-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{DATA.prev.title}</span>
            </Link>
          ) : <div className="flex-1" />}
          {DATA.next && (
            <Link href={`/spotting/pathology/${DATA.next.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-sky-50 hover:text-sky-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 pt-4 sm:pt-0 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          <Link href="/spotting" className="hover:text-sky-600 transition-colors">Spotting Centre</Link>
          <ChevronRight size={13} className="text-gray-300" />
          <Link href="/spotting/pathology/lessons" className="hover:text-sky-600 transition-colors">Pathology Lessons</Link>
          <ChevronRight size={13} className="text-gray-300" />
                  ...
                  {ALL_LESSONS.map(l => (
                    <li key={l.id}>
                      <Link href={`/spotting/pathology/${l.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${l.id === "bph"
                        ...
                        <ChevronRight size={11} className={`shrink-0 transition-opacity ${l.id === "bph" ? "opacity-100" : "opacity-0 group-hover:opacity-60"}`} />
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex-1 min-w-0 w-full space-y-5">
              ...
              <div className="p-4 sm:p-6">
                ...
                <ul className="space-y-2">
                  {DATA.points.map((point, i) => (
                    <li key={i} className="flex items-start gap-3 p-2 rounded-lg bg-sky-50/60 border border-sky-100">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {DATA.prev ? (
                <Link href={`/spotting/pathology/${DATA.prev.id}`}
                  ...
                  <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${GRAD}`} />
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-sky-50 border border-sky-100 flex items-center justify-center flex-shrink-0">
                    <ChevronLeft size={15} className="text-sky-600" />
                  </div>
                  <div className="min-w-0">
              ...
              {DATA.next ? (
                <Link href={`/spotting/pathology/${DATA.next.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-sky-50 border border-sky-100 flex items-center justify-center flex-shrink-0">
                    <ChevronRight size={15} className="text-sky-600" />
                  </div>
                </Link>
              ) : <div />}
            </div>
            ...
            <div className={`relative rounded-2xl bg-gradient-to-r ${GRAD} overflow-hidden p-5 sm:p-7`}>
              ...
              <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <Link href="/spotting/pathology/test"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-sky-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all shrink-0">
