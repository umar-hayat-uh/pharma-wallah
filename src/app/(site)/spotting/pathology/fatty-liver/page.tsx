"use client";
// app/spotting/pathology/lessons/fatty-liver/page.tsx
...
const GRAD = "from-yellow-500 to-amber-300";
const TITLE = "Fatty Liver";
const CAT = "Hepatic Pathology";
const EMOJI = "🧈";
const VIDEO_URL = "https://youtu.be/V0pGZeNLXfI?si=_p1HE5IY3BRzEOUo";
const VIDEO_TITLE = "Fatty Liver / Hepatic Steatosis — Pathology Lecture";
const PO_URL = "https://www.pathologyoutlines.com/topic/livertumorfattychange.html";
...
const DATA: {
  ...
  definition: "Fatty liver (hepatic steatosis) is the abnormal accumulation of lipid (predominantly triglycerides) within hepatocytes, visible as clear cytoplasmic vacuoles displacing the nucleus to the periphery. It is the histological manifestation of NAFLD or alcoholic liver disease and may progress through steatohepatitis to cirrhosis.",
  generalFeatures: [
    "Macro‑vesicular steatosis: large single fat vacuole displacing nucleus to cell periphery",
    "Ballooned hepatocytes: swollen, pale, rarefied cytoplasm indicating cell injury",
    "Dilated central vein",
    "Micro‑vesicular steatosis (severe cases): multiple small vacuoles, nucleus central",
    "Perivenular accentuation (alcoholic); pan‑lobular (NAFLD)",
    "Mallory–Denk bodies (eosinophilic intracytoplasmic inclusions) in NASH",
  ],
  sites: [
    "Centrilobular (zone 3): alcoholic and NAFLD/NASH — most common pattern",
    "Periportal (zone 1): malnutrition, pregnancy, toxins",
    "Pan‑lobular: severe systemic disease",
  ],
  pathophysiology: "Two-hit hypothesis (NAFLD): first hit — lipid accumulation (insulin resistance → increased FFA delivery to liver); second hit — oxidative stress, mitochondrial dysfunction, and inflammatory cytokines (TNF‑α, IL‑6) drive steatohepatitis and fibrosis. Alcohol causes direct mitochondrial toxicity, NAD depletion, and increased fatty acid synthesis.",
  etiology: [
    "NAFLD/NASH: obesity, type 2 diabetes, metabolic syndrome — most common",
    "Alcohol excess: >21 units/week (males), >14 units/week (females)",
    "Drugs: amiodarone, methotrexate, corticosteroids, tamoxifen, valproate",
    "Acute fatty liver of pregnancy (AFLP): obstetric emergency",
    "Reye syndrome, Wilson's disease, starvation/TPN",
  ],
  clinicalFeatures: [
    "Often asymptomatic — incidental finding on imaging",
    "Hepatomegaly: smooth, soft liver",
    "Elevated ALT/AST (AST:ALT <2 in NAFLD; >2 suggests alcohol)",
    "Fatigue and right upper quadrant discomfort",
    "NASH may progress: cirrhosis, portal hypertension, HCC",
  ],
  diagnosis: [
    "Ultrasound: bright (echogenic) liver; sensitivity 60–94%",
    "Fibroscan (transient elastography): steatosis + fibrosis assessment",
    "Liver biopsy: gold standard — NAS score for NAFLD (steatosis + ballooning + inflammation)",
    "LFTs, fasting glucose, lipid profile, HbA1c",
  ],
  treatment: [
    "Weight loss (>7–10%): most effective intervention for NAFLD",
    "Alcohol abstinence: essential for alcoholic fatty liver",
    "Vitamin E: for non‑diabetic NASH (improves NAS score)",
    "GLP‑1 agonists (semaglutide): emerging evidence for NASH regression",
    "Liver transplantation: for end‑stage NASH cirrhosis",
  ],
  points: [
    "Macrovesicular steatosis (large vacuole, peripheral nucleus)",
  ...
  images: [
    "/images/spotting/pathology/fatty-liver-sample.jpg",
    "/images/spotting/pathology/fatty-liver-high.jpg",
  ],
  prev: { id: "adenocarcinoma", title: "Adenocarcinoma" },
  next: { id: "cvc-liver", title: "Chronic Venous Congestion (Liver)" },
};
...
// ─── Main page component ─────────────────────────────────────────────────────
export default function PathologyFattyLiverPage() {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [lightbox, setLightbox] = useState<string | null>(null);
  const lessonIdx = ALL_LESSONS.findIndex(l => l.id === "fatty-liver");
  ...
  useEffect(() => {
    trackSpotting({
      lessonId: "fatty-liver",   // change this per page
      category: "pathology",            // "pathology" | "histology" | "powder-microscopy"
      completed: true,
    });
    trackActivity({
      type: "spotting",
      label: "Opened: Fatty Liver", // change label accordingly
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {/* Navigation drawer */}
      <LessonNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} currentId="fatty-liver" />
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5 pt-[2.625rem]">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href="/spotting/pathology/lessons" className="hover:text-amber-600 shrink-0 font-medium">Pathology</Link>
                  ...
                  {ALL_LESSONS.map(l => (
                    <li key={l.id}>
                      <Link href={`/spotting/pathology/${l.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${l.id === "fatty-liver"
                        ...
                        <ChevronRight size={11} className={`shrink-0 transition-opacity ${l.id === "fatty-liver" ? "opacity-100" : "opacity-0 group-hover:opacity-60"}`} />
                      </Link>
                    </li>
                  ))}
                </ul>
