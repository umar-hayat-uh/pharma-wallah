...
// app/courses/sem-1/pharmaceutical-organic-chemistry/[unit]/page.tsx
...
const BASE_PATH = `/courses/${ORG_CHEM_META.semesterSlug}/${ORG_CHEM_META.slug}`;
...
function ScrollTable({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative my-5 rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {canR && <div className="absolute top-2 right-2 z-20 bg-emerald-600 text-white text-[9px] font-bold px-2 py-0.5 rounded-full pointer-events-none sm:hidden">scroll →</div>}
...
// ─── Markdown components — emerald/teal theme ─────────────────────────────────
const mdComponents = {
  h1: ({ children }: any) => (
    <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent mt-8 mb-4 pb-3 border-b-2 border-emerald-100 break-words">{children}</h1>
  ),
  h2: ({ children }: any) => (
  ...
  h3: ({ children }: any) => (
    <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent mt-6 mb-3 break-words">{children}</h3>
  ),
  h4: ({ children }: any) => (
  ...
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-4 border-emerald-400 bg-emerald-50/60 px-4 py-3 rounded-r-xl my-4 text-gray-700 text-sm sm:text-base italic break-words">{children}</blockquote>
  ),
  table:  ({ children }: any) => <ScrollTable>{children}</ScrollTable>,
  thead:  ({ children }: any) => <thead className="bg-gradient-to-r from-emerald-50 to-teal-50">{children}</thead>,
  ...
  tr:     ({ children }: any) => <tr className="hover:bg-emerald-50/30 transition-colors">{children}</tr>,
  code:   ({ inline, children }: any) =>
    inline
      ? <code className="bg-gray-100 text-emerald-700 px-1.5 py-0.5 rounded text-xs font-mono break-all">{children}</code>
...
// ═══════════════════════════════════════════════════════════════════════════════
export default function OrganicChemistryUnitPage({ params }: PageProps) {
  const { unit: unitSlug } = params;
  const unitIdx  = OrgChemUnits.findIndex(u => u.id === unitSlug);
  const unit     = OrgChemUnits[unitIdx];
  const prevUnit = unitIdx > 0 ? OrgChemUnits[unitIdx - 1] : null;
  const nextUnit = unitIdx < OrgChemUnits.length - 1 ? OrgChemUnits[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/pharmaceutical-organic-chemistry/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      // Branded emerald header bar
      const addHeader = (doc: typeof pdf, p: number, t: number) => {
        doc.setFillColor(5, 150, 105); // emerald-600
        doc.rect(0, 0, pageW, 10, "F");
        doc.setTextColor(255, 255, 255); doc.setFontSize(7);
        doc.setFont("helvetica", "bold");
        doc.text(`PHARMAWALLAH  ·  ${ORG_CHEM_META.title.toUpperCase()}`, margin, 6.5);
      ...
      console.error(err); alert("PDF generation failed. Please try again.");
  ...
  if (!unit) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <Link href={BASE_PATH} className="text-emerald-600 text-sm font-semibold hover:underline">← Back to Organic Chemistry</Link>
      </div>
    </div>
  );
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            ...
            <span className="text-emerald-700 font-semibold truncate">{unit.shortTitle}</span>
          </div>
                ...
                <li key={u.id}>
                  <Link href={`${BASE_PATH}/${u.id}`} onClick={() => setMobileNavOpen(false)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs transition-all ${
                      u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold` : "text-gray-700 hover:bg-emerald-50"
                    }`}>
        ...
        <div className="flex border-t border-gray-100">
          {prevUnit ? (
            <Link href={`${BASE_PATH}/${prevUnit.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{prevUnit.emoji} {prevUnit.shortTitle}</span>
            </Link>
          ) : <div className="flex-1" />}
          {nextUnit && (
            <Link href={`${BASE_PATH}/${nextUnit.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[
            ...
            { href: `/courses/${ORG_CHEM_META.semesterSlug}`, label: ORG_CHEM_META.semester },
            { href: BASE_PATH,  label: "Pharmaceutical Organic Chemistry" },
          ].map(({ href, label }) => (
          ...
          <span className="text-emerald-700 font-semibold">{unit.shortTitle}</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-8 items-start w-full">
            ...
            <div className="sticky top-6 space-y-4">
              ...
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Organic Chemistry</span>
                </div>
                <p className="font-bold text-xs leading-snug mt-1">{ORG_CHEM_META.semester} · {ORG_CHEM_META.subjectCode}</p>
              </div>
                    ...
                    <li key={u.id}>
                      <Link href={`${BASE_PATH}/${u.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${
                          u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold shadow-sm` : "text-gray-600 hover:bg-emerald-50 hover:text-emerald-700"
                        }`}>
                        <span>{u.emoji}</span>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3 space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Clock size={12} className="text-emerald-500 shrink-0" /><span>{unit.readTime} min read</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <GraduationCap size={12} className="text-emerald-500 shrink-0" /><span>Difficulty: <strong>{unit.difficulty}</strong></span>
                </div>
              </div>
          ...
          <div className="flex-1 min-w-0 w-full">
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Pharmaceutical Organic Chemistry · {ORG_CHEM_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Unit ${unitIdx + 1} of ${OrgChemUnits.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
            ...
            {/* ══ PRINTABLE AREA — only this div is captured for PDF ══ */}
                ...
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-emerald-600 font-mono">
                        public/content/pharmaceutical-organic-chemistry/{unitSlug}.md
                      </code>
                    </p>
            ...
            {/* Download button — outside printRef so it won't appear in PDF */}
            {!loading && !error && (
