...
// app/courses/sem-1/pharmaceutical-organic-chemistry/page.tsx
...
import {
  ...
  Clock, Tag, Zap, Layers, GraduationCap, ArrowRight, Atom,
} from "lucide-react";
import { OrgChemUnits, ORG_CHEM_META, ORG_CHEM_DIFF_BADGE } from "@/app/api/organic-chemistry-data";
...
const HERO_GRAD = "from-emerald-600 to-teal-500";
const BASE_PATH = `/courses/${ORG_CHEM_META.semesterSlug}/${ORG_CHEM_META.slug}`;

export default function OrganicChemistryUnitsPage() {
  const [search,     setSearch]     = useState("");
  const [activeDiff, setActiveDiff] = useState("All");
  const diffs = ["All", "Easy", "Medium", "Hard"];

  const filtered = useMemo(() =>
    OrgChemUnits.filter(u =>
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {bgIcons.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-emerald-100 z-0 hidden md:block" style={{ top, left }}>
      ...
      <div className={`relative bg-gradient-to-r ${HERO_GRAD} overflow-hidden`}>
        ...
        <div className="absolute right-10 bottom-6 opacity-[0.10] pointer-events-none hidden sm:block">
          <Atom size={110} className="text-white" />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20">
          ...
          <div className="flex items-center gap-2 text-white/70 text-xs font-semibold mb-4 flex-wrap">
            ...
            <Link href={`/courses/${ORG_CHEM_META.semesterSlug}`} className="hover:text-white transition">
              {ORG_CHEM_META.semester}
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white">Organic Chemistry</span>
          </div>
          ...
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 border border-white/30 text-white text-xs font-bold uppercase tracking-widest mb-5">
            <Zap className="w-3.5 h-3.5" /> {ORG_CHEM_META.semester} · {ORG_CHEM_META.subjectCode}
          </span>

          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white mb-2 tracking-tight leading-tight">
            Pharmaceutical
            <span className="block text-teal-200 mt-1">Organic Chemistry</span>
          </h1>
          <p className="text-white/80 text-sm sm:text-base max-w-2xl mb-8">
            {ORG_CHEM_META.description}
          </p>

          <div className="flex flex-wrap gap-3 mb-8">
            <Link href={`${BASE_PATH}/${OrgChemUnits[0].id}`}
          ...
          <div className="flex flex-wrap gap-6 sm:gap-10">
            {[
              { n: String(OrgChemUnits.length),                                           l: "Units"  },
              { n: String(OrgChemUnits.filter(u => u.difficulty === "Easy").length),   l: "Easy"   },
              { n: String(OrgChemUnits.filter(u => u.difficulty === "Medium").length), l: "Medium" },
              { n: String(OrgChemUnits.filter(u => u.difficulty === "Hard").length),   l: "Hard"   },
            ].map(({ n, l }) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {filtered.map((unit) => (
              <Link key={unit.id} href={`${BASE_PATH}/${unit.id}`}
                className="group relative flex flex-col rounded-2xl border border-gray-200 bg-white overflow-hidden hover:border-emerald-300 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 h-full">
                ...
                <div className="flex flex-col flex-1 p-4 sm:p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${ORG_CHEM_DIFF_BADGE[unit.difficulty]}`}>
                      {unit.difficulty}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className={`text-[10px] font-extrabold uppercase tracking-wide ${unit.color.iconText}`}>
                      Unit {OrgChemUnits.indexOf(unit) + 1} of {OrgChemUnits.length}
                    </span>
                  </div>
                  <h3 className="font-extrabold text-gray-900 text-sm sm:text-base mb-2 leading-snug group-hover:text-emerald-700 transition-colors">
                    {unit.shortTitle}
                  </h3>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-5">
            <div>
              ...
              <p className="text-white/80 text-xs sm:text-sm">
                Start with <span className="font-bold text-white">Unit 1 — Basic Concepts</span> and work your way up.
              </p>
            </div>
            <Link href={`${BASE_PATH}/${OrgChemUnits[0].id}`}
              className="inline-flex items-center gap-2 px-5 sm:px-6 py-3 rounded-xl bg-white text-emerald-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 shrink-0">
