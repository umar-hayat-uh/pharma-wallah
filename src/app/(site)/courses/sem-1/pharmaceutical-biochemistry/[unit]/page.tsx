...
// ── Dashboard integration: tracks unit reads, time spent, activity feed ──
    ...
    <section className="min-h-screen bg-white relative" style={{ overflowX: "hidden" }}>
              ...
              {BiochemUnits.map(u => (
                <li key={u.id}>
                  <Link href={`${BASE_PATH}/${u.id}`} onClick={() => setMobileNavOpen(false)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs transition-all ${u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold` : "text-gray-700 hover:bg-blue-50"}`}>
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[{ href: "/", label: "Home" }, { href: "/courses", label: "Courses" }, { href: `/courses/${SUBJECT_META.semesterSlug}`, label: SUBJECT_META.semester }, { href: BASE_PATH, label: "Pharmaceutical Biochemistry" }].map(({ href, label }) => (
                  ...
                  {BiochemUnits.map(u => (
                    <li key={u.id}>
                      <Link href={`${BASE_PATH}/${u.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold shadow-sm` : "text-gray-600 hover:bg-blue-50 hover:text-blue-700"}`}>
                        <span>{u.emoji}</span>
              ...
              {unit.previewImage && !imgError && (
                <div className="mb-4 sm:mb-5 rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-gray-50">
                  ...
                  <img src={unit.previewImage} alt={`${unit.shortTitle} overview`} crossOrigin="anonymous"
                    className="w-full max-h-72 sm:max-h-96 object-contain bg-gray-50 p-2 sm:p-4" onError={() => setImgError(true)} />
                </div>
              )}
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3 sm:p-5 md:p-8 overflow-hidden" style={{ wordBreak: "break-word", overflowWrap: "break-word" }}>
                ...
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-blue-600 font-mono">public/content/pharmaceutical-biochemistry/{unitSlug}.md</code></p>
                    <Link href={BASE_PATH} className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r ${GRAD} text-white text-xs font-extrabold`}>← Back to all units</Link>
                  </div>
                )}
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              {prevUnit ? (
                <Link href={`${BASE_PATH}/${prevUnit.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center flex-shrink-0"><ChevronLeft size={15} className="text-blue-600" /></div>
              ...
              {nextUnit ? (
                <Link href={`${BASE_PATH}/${nextUnit.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center flex-shrink-0"><ChevronRight size={15} className="text-blue-600" /></div>
                </Link>
              ) : <div />}
            </div>
          </div>
