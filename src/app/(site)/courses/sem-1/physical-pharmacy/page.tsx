...
// app/courses/sem-4/physical-pharmacy/page.tsx
...
import { PhysPharmUnits, PHYSPHARM_META, PHYSPHARM_DIFF_BADGE } from "@/app/api/physical-pharmacy-data";
...
const HERO_GRAD = "from-indigo-600 to-violet-500";
const BASE_PATH = `/courses/${PHYSPHARM_META.semesterSlug}/${PHYSPHARM_META.slug}`;

export default function PhysicalPharmacyUnitsPage() {
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {bgIcons.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-indigo-100 z-0 hidden md:block" style={{ top, left }}>
      ...
      <div className={`relative bg-gradient-to-r ${HERO_GRAD} overflow-hidden`}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20">
          ...
          <div className="flex items-center gap-2 text-white/70 text-xs font-semibold mb-4 flex-wrap">
            ...
            <Link href={`/courses/${PHYSPHARM_META.semesterSlug}`} className="hover:text-white transition">
              {PHYSPHARM_META.semester}
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white">Physical Pharmacy</span>
          </div>
          ...
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 border border-white/30 text-white text-xs font-bold uppercase tracking-widest mb-5">
            <Zap className="w-3.5 h-3.5" /> {PHYSPHARM_META.semester} · {PHYSPHARM_META.subjectCode}
          </span>

          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white mb-2 tracking-tight leading-tight">
            Physical
            <span className="block text-violet-200 mt-1">Pharmacy</span>
          </h1>
          <p className="text-white/80 text-sm sm:text-base max-w-2xl mb-8">
            {PHYSPHARM_META.description}
          </p>

          <div className="flex flex-wrap gap-3 mb-8">
            <Link href={`${BASE_PATH}/${PhysPharmUnits[0].id}`}
          ...
          <div className="flex flex-wrap gap-6 sm:gap-10">
            {[
              { n: String(PhysPharmUnits.length),                                             l: "Units"  },
              { n: String(PhysPharmUnits.filter(u => u.difficulty === "Easy").length),   l: "Easy"   },
              { n: String(PhysPharmUnits.filter(u => u.difficulty === "Medium").length), l: "Medium" },
              { n: String(PhysPharmUnits.filter(u => u.difficulty === "Hard").length),   l: "Hard"   },
            ].map(({ n, l }) => (
              ...
              >
                ...
                <div className="flex flex-col flex-1 p-4 sm:p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${PHYSPHARM_DIFF_BADGE[unit.difficulty]}`}>
                      {unit.difficulty}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className={`text-[10px] font-extrabold uppercase tracking-wide ${unit.color.iconText}`}>
                      Unit {PhysPharmUnits.indexOf(unit) + 1} of {PhysPharmUnits.length}
                    </span>
                  </div>

                  <h3 className="font-extrabold text-gray-900 text-sm sm:text-base mb-2 leading-snug group-hover:text-indigo-700 transition-colors">
                    {unit.shortTitle}
                  </h3>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-5">
            <div>
              ...
              <p className="text-white/80 text-xs sm:text-sm">
                Start with <span className="font-bold text-white">Unit 1 — Intro to Pharmacy & History</span> and work your way up.
              </p>
            </div>
            <Link href={`${BASE_PATH}/${PhysPharmUnits[0].id}`}
              className="inline-flex items-center gap-2 px-5 sm:px-6 py-3 rounded-xl bg-white text-indigo-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 shrink-0">
