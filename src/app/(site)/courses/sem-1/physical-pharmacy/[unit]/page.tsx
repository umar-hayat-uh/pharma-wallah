...
// app/courses/sem-4/physical-pharmacy/[unit]/page.tsx
...
function ScrollTable({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative my-5 rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {canR && <div className="absolute top-2 right-2 z-20 bg-indigo-600 text-white text-[9px] font-bold px-2 py-0.5 rounded-full pointer-events-none sm:hidden">scroll →</div>}
...
// ─── Markdown components — indigo/violet theme ────────────────────────────────
const mdComponents = {
  h1: ({ children }: any) => (
    <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-indigo-600 to-violet-500 bg-clip-text text-transparent mt-8 mb-4 pb-3 border-b-2 border-indigo-100 break-words">{children}</h1>
  ),
  h2: ({ children }: any) => (
  ...
  h3: ({ children }: any) => (
    <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-indigo-600 to-violet-500 bg-clip-text text-transparent mt-6 mb-3 break-words">{children}</h3>
  ),
  h4: ({ children }: any) => (
...
export default function PhysicalPharmacyUnitPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/content/physical-pharmacy/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      const addHeader = (doc: typeof pdf, pageNum: number, total: number) => {
        ...
        doc.text(`PHARMAWALLAH  ·  ${PHYSPHARM_META.title.toUpperCase()}`, margin, 6.5);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href={BASE_PATH} className="text-indigo-600 text-sm font-semibold hover:underline">← Back to Physical Pharmacy</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5 max-w-full">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-indigo-600 shrink-0 font-medium">Physical Pharmacy</Link>
            <ChevronRight size={11} className="shrink-0 mx-0.5 text-gray-300" />
            <span className="text-indigo-600 font-semibold truncate">{unit.shortTitle}</span>
          </div>
        ...
        <div className="flex border-t border-gray-100">
          {prevUnit ? (
            <Link href={`${BASE_PATH}/${prevUnit.id}`}
              className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors min-w-0">
          ...
          {nextUnit && (
            <Link href={`${BASE_PATH}/${nextUnit.id}`}
              className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors border-l border-gray-100 min-w-0">
            ...
            <div className="sticky top-6 space-y-4">
              ...
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Physical Pharmacy</span>
                </div>
                <p className="font-bold text-xs leading-snug mt-1">{PHYSPHARM_META.semester} · {PHYSPHARM_META.subjectCode}</p>
              </div>
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Physical Pharmacy · {PHYSPHARM_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Unit ${unitIdx + 1} of ${PhysPharmUnits.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
