...
// app/courses/sem-1/physiology-histology-1/[unit]/page.tsx
...
function ScrollTable({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative my-5 rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {canR && <div className="absolute top-2 right-2 z-20 bg-orange-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full pointer-events-none sm:hidden">scroll →</div>}
...
const mdComponents = {
  h1: ({ children }: any) => (
    <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-orange-500 to-rose-500 bg-clip-text text-transparent mt-8 mb-4 pb-3 border-b-2 border-orange-100 break-words">{children}</h1>
  ),
  h2: ({ children }: any) => (
  ...
  h3: ({ children }: any) => (
    <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-orange-500 to-rose-500 bg-clip-text text-transparent mt-6 mb-3 break-words">{children}</h3>
  ),
  h4: ({ children }: any) => (
    <h4 className="text-sm sm:text-base font-semibold text-orange-700 mt-5 mb-2 break-words">{children}</h4>
  ),
  ...
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-4 border-orange-400 bg-orange-50/60 px-4 py-3 rounded-r-xl my-4 text-gray-700 text-sm sm:text-base italic break-words">{children}</blockquote>
  ),
  table:  ({ children }: any) => <ScrollTable>{children}</ScrollTable>,
  thead:  ({ children }: any) => <thead className="bg-gradient-to-r from-orange-50 to-rose-50">{children}</thead>,
  ...
  td:     ({ children }: any) => <td className="px-3 py-2.5 border border-gray-200 text-xs text-gray-700 whitespace-normal min-w-[90px]">{children}</td>,
  tr:     ({ children }: any) => <tr className="hover:bg-orange-50/30 transition-colors">{children}</tr>,
  code:   ({ inline, children }: any) =>
    inline
      ? <code className="bg-gray-100 text-orange-700 px-1.5 py-0.5 rounded text-xs font-mono break-all">{children}</code>
      : <pre className="bg-gray-900 text-orange-300 rounded-2xl p-4 overflow-x-auto text-xs font-mono my-4 leading-relaxed"><code>{children}</code></pre>,
  hr: () => <hr className="my-8 border-gray-200" />,
};

// ═══════════════════════════════════════════════════════════════════════════════
export default function PhysiologyUnitPage({ params }: PageProps) {
  const { unit: unitSlug } = params;

  const unitIdx  = PhysUnits.findIndex(u => u.id === unitSlug);
  const unit     = PhysUnits[unitIdx];
  const prevUnit = unitIdx > 0 ? PhysUnits[unitIdx - 1] : null;
  const nextUnit = unitIdx < PhysUnits.length - 1 ? PhysUnits[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/physiology-histology-1/${unitSlug}.md`)
  ...
  // ── PDF generation — captures only the content div ──────────────────────────
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      // Temporarily expand the element to full scroll height so nothing is clipped
      ...
      const canvas = await html2canvas(el, {
        ...
        windowWidth: 900,          // simulate desktop width for layout
      });
      ...
      const addHeader = (doc: typeof pdf, pageNum: number, totalPages: number) => {
        doc.setFillColor(234, 88, 12);   // orange-600
        doc.rect(0, 0, pageW, 10, "F");
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(7);
        doc.setFont("helvetica", "bold");
        doc.text(`PHARMAWALLAH  ·  ${PHYSIO_META.title.toUpperCase()}`, margin, 6.5);
        doc.setFont("helvetica", "normal");
        doc.text(`Page ${pageNum} of ${totalPages}`, pageW - margin, 6.5, { align: "right" });
      };
      ...
      const ratio       = contentW / (imgW / (canvas.width / contentW));
      const scaledW     = contentW;
      const scaledH     = (imgH / imgW) * scaledW;
      const headerSpace = 12;   // mm space after header
      const usableH     = contentH - headerSpace;
      ...
      for (let page = 0; page < totalPages; page++) {
        ...
        const srcY      = (page * usableH * imgH) / scaledH;
        const sliceH    = Math.min((usableH * imgH) / scaledH, imgH - srcY);
        const slicePxH  = (sliceH / imgH) * canvas.height;
        const slicePxY  = (srcY / imgH) * canvas.height;
        ...
        sliceCtx.drawImage(canvas, 0, slicePxY, canvas.width, slicePxH, 0, 0, canvas.width, slicePxH);

        const sliceData = sliceCanvas.toDataURL("image/jpeg", 0.95);
        const drawH     = (slicePxH / canvas.height) * scaledH;
        pdf.addImage(sliceData, "JPEG", margin, margin + headerSpace, scaledW, drawH);
      }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href={BASE_PATH} className="text-orange-600 text-sm font-semibold hover:underline">← Back to Physiology</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="min-h-screen bg-white relative" style={{ overflowX: "hidden" }}>
      ...
      {bgIconDefs.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-orange-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5 max-w-full">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-orange-600 shrink-0 font-medium">Physiology</Link>
            <ChevronRight size={11} className="shrink-0 mx-0.5 text-gray-300" />
            <span className="text-orange-600 font-semibold truncate">{unit.shortTitle}</span>
          </div>
                ...
                <li key={u.id}>
                  <Link href={`${BASE_PATH}/${u.id}`} onClick={() => setMobileNavOpen(false)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs transition-all ${
                      u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold` : "text-gray-700 hover:bg-orange-50"
                    }`}>
        ...
        <div className="flex border-t border-gray-100">
          {prevUnit ? (
            <Link href={`${BASE_PATH}/${prevUnit.id}`}
              className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-orange-50 hover:text-orange-600 transition-colors min-w-0">
          ...
          {nextUnit && (
            <Link href={`${BASE_PATH}/${nextUnit.id}`}
              className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-orange-50 hover:text-orange-600 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[
            ...
            { href: `/courses/${PHYSIO_META.semesterSlug}`, label: PHYSIO_META.semester },
            { href: BASE_PATH, label: "Physiology & Histology-I" },
          ].map(({ href, label }) => (
            <span key={href} className="flex items-center gap-1.5">
              <Link href={href} className="hover:text-orange-600 transition-colors">{label}</Link>
              <ChevronRight size={13} className="text-gray-300" />
            </span>
          ))}
          <span className="text-orange-600 font-semibold">{unit.shortTitle}</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-8 items-start w-full">
            ...
            <div className="sticky top-6 space-y-4">
              ...
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Physiology & Histology-I</span>
                </div>
                <p className="font-bold text-xs leading-snug mt-1">{PHYSIO_META.semester} · {PHYSIO_META.subjectCode}</p>
              </div>
                    ...
                    <li key={u.id}>
                      <Link href={`${BASE_PATH}/${u.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${
                          u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold shadow-sm` : "text-gray-600 hover:bg-orange-50 hover:text-orange-700"
                        }`}>
                        <span>{u.emoji}</span>
          ...
          <div className="flex-1 min-w-0 w-full">
            ...
            {/* Hero banner */}
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-6 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Physiology & Histology-I · {PHYSIO_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Unit ${unitIdx + 1} of ${PhysUnits.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
                ...
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-orange-600 font-mono">public/content/physiology-histology-1/{unitSlug}.md</code>
                    </p>
                    <Link href={BASE_PATH}
            ...
            {/* Download button — outside printRef so it doesn't appear in PDF */}
            {!loading && !error && (
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              {prevUnit ? (
                <Link href={`${BASE_PATH}/${prevUnit.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-orange-50 border border-orange-100 flex items-center justify-center flex-shrink-0">
                    <ChevronLeft size={15} className="text-orange-600" />
                  </div>
              ...
              {nextUnit ? (
                <Link href={`${BASE_PATH}/${nextUnit.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-orange-50 border border-orange-100 flex items-center justify-center flex-shrink-0">
                    <ChevronRight size={15} className="text-orange-600" />
                  </div>
                </Link>
              ) : <div />}
            </div>
