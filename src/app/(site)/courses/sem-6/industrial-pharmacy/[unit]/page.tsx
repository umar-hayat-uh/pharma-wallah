...
// app/courses/sem-6/industrial-pharmacy/[unit]/page.tsx
...
import { IndustrialPharmUnits, INDUSTRIAL_PHARM_META } from "../../../../../api/industrial-pharmacy-data";
...
const BASE_PATH = `/courses/${INDUSTRIAL_PHARM_META.semesterSlug}/${INDUSTRIAL_PHARM_META.slug}`;
...
function ScrollTable({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative my-5 rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {canR && <div className="absolute top-2 right-2 z-20 bg-slate-700 text-white text-[9px] font-bold px-2 py-0.5 rounded-full pointer-events-none sm:hidden">scroll →</div>}
...
// ─── Markdown components — slate/blue theme ───────────────────────────────────
const mdComponents = {
  h1: ({ children }: any) => (
    <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-slate-700 to-blue-600 bg-clip-text text-transparent mt-8 mb-4 pb-3 border-b-2 border-slate-100 break-words">{children}</h1>
  ),
  h2: ({ children }: any) => (
  ...
  h3: ({ children }: any) => (
    <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-slate-700 to-blue-600 bg-clip-text text-transparent mt-6 mb-3 break-words">{children}</h3>
  ),
  h4: ({ children }: any) => (
    <h4 className="text-sm sm:text-base font-semibold text-blue-700 mt-5 mb-2 break-words">{children}</h4>
  ),
  ...
  thead:  ({ children }: any) => <thead className="bg-gradient-to-r from-slate-50 to-blue-50">{children}</thead>,
...
// ═══════════════════════════════════════════════════════════════════════════════
export default function IndustrialPharmacyUnitPage({ params }: PageProps) {
  const { unit: unitSlug } = params;
  const unitIdx  = IndustrialPharmUnits.findIndex(u => u.id === unitSlug);
  const unit     = IndustrialPharmUnits[unitIdx];
  const prevUnit = unitIdx > 0 ? IndustrialPharmUnits[unitIdx - 1] : null;
  const nextUnit = unitIdx < IndustrialPharmUnits.length - 1 ? IndustrialPharmUnits[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/industrial-pharmacy/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      // Branded slate-700 header bar
      const addHeader = (doc: typeof pdf, p: number, t: number) => {
        doc.setFillColor(51, 65, 85); // slate-700
        doc.rect(0, 0, pageW, 10, "F");
        doc.setTextColor(255, 255, 255); doc.setFontSize(7);
        doc.setFont("helvetica", "bold");
        doc.text(`PHARMAWALLAH  ·  ${INDUSTRIAL_PHARM_META.title.toUpperCase()}`, margin, 6.5);
      ...
      for (let page = 0; page < totalPages; page++) {
        ...
        pdf.addImage(
          ...
          margin, margin + headerSpace,
          scaledW, (slicePxH / canvas.height) * scaledH
        );
      }
  ...
  if (!unit) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <Link href={BASE_PATH} className="text-blue-600 text-sm font-semibold hover:underline">
          ← Back to Industrial Pharmacy
        </Link>
      </div>
    </div>
  );
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-blue-600 shrink-0 font-medium">Industrial Pharm</Link>
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[
            ...
            { href: `/courses/${INDUSTRIAL_PHARM_META.semesterSlug}`, label: INDUSTRIAL_PHARM_META.semester },
            { href: BASE_PATH,  label: "Industrial Pharmacy" },
          ].map(({ href, label }) => (
            ...
            <div className="sticky top-6 space-y-4">
              ...
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Industrial Pharmacy</span>
                </div>
                <p className="font-bold text-xs leading-snug mt-1">
                  {INDUSTRIAL_PHARM_META.semester} · {INDUSTRIAL_PHARM_META.subjectCode}
                </p>
              </div>
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Industrial Pharmacy · {INDUSTRIAL_PHARM_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Unit ${unitIdx + 1} of ${IndustrialPharmUnits.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
                  ...
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      ...
                      <code className="bg-gray-100 px-1.5 py-0.5 rounded text-blue-600 font-mono">
                        public/content/industrial-pharmacy/{unitSlug}.md
                      </code>
                    </p>
                    <Link href={BASE_PATH}
