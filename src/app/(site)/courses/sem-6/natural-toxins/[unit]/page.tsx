...
// app/courses/sem-6/natural-toxins/[unit]/page.tsx
...
import { NaturalToxinsUnits, NATURAL_TOXINS_META } from "@/app/api/natural-toxins-data";
...
const BASE_PATH = `/courses/${NATURAL_TOXINS_META.semesterSlug}/${NATURAL_TOXINS_META.slug}`;
...
function ScrollTable({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative my-5 rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {canR && <div className="absolute top-2 right-2 z-20 bg-green-700 text-white text-[9px] font-bold px-2 py-0.5 rounded-full pointer-events-none sm:hidden">scroll →</div>}
...
const mdComponents = {
  h1: ({ children }: any) => (
    <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-green-700 to-emerald-500 bg-clip-text text-transparent mt-8 mb-4 pb-3 border-b-2 border-green-100 break-words">{children}</h1>
  ),
  h2: ({ children }: any) => (
  ...
  h3: ({ children }: any) => (
    <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-green-700 to-emerald-500 bg-clip-text text-transparent mt-6 mb-3 break-words">{children}</h3>
  ),
  h4: ({ children }: any) => (
    <h4 className="text-sm sm:text-base font-semibold text-emerald-700 mt-5 mb-2 break-words">{children}</h4>
  ),
  ...
  li: ({ children }: any) => (
    <li className="flex gap-2.5 text-gray-700 text-sm sm:text-base leading-relaxed">
      <span className="mt-[7px] w-1.5 h-1.5 rounded-full bg-green-600 flex-shrink-0" />
  ...
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-4 border-green-500 bg-green-50/60 px-4 py-3 rounded-r-xl my-4 text-gray-700 text-sm sm:text-base italic break-words">{children}</blockquote>
  ),
  table:  ({ children }: any) => <ScrollTable>{children}</ScrollTable>,
  thead:  ({ children }: any) => <thead className="bg-gradient-to-r from-green-50 to-emerald-50">{children}</thead>,
  ...
  tr:     ({ children }: any) => <tr className="hover:bg-green-50/30 transition-colors">{children}</tr>,
  code:   ({ inline, children }: any) =>
    inline
      ? <code className="bg-gray-100 text-green-700 px-1.5 py-0.5 rounded text-xs font-mono break-all">{children}</code>
      : <pre className="bg-gray-900 text-emerald-300 rounded-2xl p-4 overflow-x-auto text-xs font-mono my-4 leading-relaxed"><code>{children}</code></pre>,
  hr: () => <hr className="my-8 border-gray-200" />,
};

export default function NaturalToxinsUnitPage({ params }: PageProps) {
  const { unit: unitSlug } = params;
  const unitIdx  = NaturalToxinsUnits.findIndex(u => u.id === unitSlug);
  const unit     = NaturalToxinsUnits[unitIdx];
  const prevUnit = unitIdx > 0 ? NaturalToxinsUnits[unitIdx - 1] : null;
  const nextUnit = unitIdx < NaturalToxinsUnits.length - 1 ? NaturalToxinsUnits[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/natural-toxins/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      const addHeader = (doc: typeof pdf, p: number, t: number) => {
        ...
        doc.text(`PHARMAWALLAH  ·  ${NATURAL_TOXINS_META.title.toUpperCase()}`, margin, 6.5);
  ...
  if (!unit) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <Link href={BASE_PATH} className="text-green-700 text-sm font-semibold hover:underline">← Back to Natural Toxins</Link>
      </div>
    </div>
  );
  ...
  return (
    <section className="min-h-screen bg-white relative" style={{ overflowX: "hidden" }}>
      ...
      {bgIconDefs.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-green-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-green-700 shrink-0 font-medium">Natural Toxins</Link>
            <ChevronRight size={11} className="shrink-0 mx-0.5 text-gray-300" />
            <span className="text-green-700 font-semibold truncate">{unit.shortTitle}</span>
          </div>
                ...
                <li key={u.id}>
                  <Link href={`${BASE_PATH}/${u.id}`} onClick={() => setMobileNavOpen(false)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs transition-all ${
                      u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold` : "text-gray-700 hover:bg-green-50"
                    }`}>
        ...
        <div className="flex border-t border-gray-100">
          {prevUnit ? (
            <Link href={`${BASE_PATH}/${prevUnit.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-green-50 hover:text-green-700 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{prevUnit.emoji} {prevUnit.shortTitle}</span>
            </Link>
          ) : <div className="flex-1" />}
          {nextUnit && (
            <Link href={`${BASE_PATH}/${nextUnit.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-green-50 hover:text-green-700 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[
            ...
            { href: `/courses/${NATURAL_TOXINS_META.semesterSlug}`, label: NATURAL_TOXINS_META.semester },
            { href: BASE_PATH,  label: "Natural Toxins" },
          ].map(({ href, label }) => (
            <span key={href} className="flex items-center gap-1.5">
              <Link href={href} className="hover:text-green-700 transition-colors">{label}</Link>
              <ChevronRight size={13} className="text-gray-300" />
            </span>
          ))}
          <span className="text-green-700 font-semibold">{unit.shortTitle}</span>
        </nav>
          ...
          <aside className="hidden lg:block w-52 xl:w-60 flex-shrink-0">
            <div className="sticky top-6 space-y-4">
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                ...
                <p className="font-bold text-xs leading-snug mt-1">{NATURAL_TOXINS_META.semester} · {NATURAL_TOXINS_META.subjectCode}</p>
              </div>
                    ...
                    <li key={u.id}>
                      <Link href={`${BASE_PATH}/${u.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${
                          u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold shadow-sm` : "text-gray-600 hover:bg-green-50 hover:text-green-700"
                        }`}>
                        <span>{u.emoji}</span>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3 space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Clock size={12} className="text-green-600 shrink-0" /><span>{unit.readTime} min read</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <GraduationCap size={12} className="text-green-600 shrink-0" /><span>Difficulty: <strong>{unit.difficulty}</strong></span>
                </div>
              </div>
              <button onClick={handleDownloadPdf} disabled={pdfLoading}
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Natural Toxins · {NATURAL_TOXINS_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Unit ${unitIdx + 1} of ${NaturalToxinsUnits.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
                ...
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-green-700 font-mono">public/content/natural-toxins/{unitSlug}.md</code>
                    </p>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              {prevUnit ? (
                <Link href={`${BASE_PATH}/${prevUnit.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-green-50 border border-green-100 flex items-center justify-center flex-shrink-0">
                    <ChevronLeft size={15} className="text-green-700" />
                  </div>
