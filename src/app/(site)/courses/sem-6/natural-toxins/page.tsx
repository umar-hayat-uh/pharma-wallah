...
// app/courses/sem-6/natural-toxins/page.tsx
...
import { NaturalToxinsUnits, NATURAL_TOXINS_META, NATURAL_TOXINS_DIFF_BADGE } from "@/app/api/natural-toxins-data";
...
const HERO_GRAD = "from-green-700 to-emerald-500";
const BASE_PATH = `/courses/${NATURAL_TOXINS_META.semesterSlug}/${NATURAL_TOXINS_META.slug}`;

export default function NaturalToxinsUnitsPage() {
  const [search,     setSearch]     = useState("");
  const [activeDiff, setActiveDiff] = useState("All");
  const diffs = ["All", "Easy", "Medium", "Hard"];

  const filtered = useMemo(() =>
    NaturalToxinsUnits.filter(u =>
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {bgIcons.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-green-100 z-0 hidden md:block" style={{ top, left }}>
      ...
      <div className={`relative bg-gradient-to-r ${HERO_GRAD} overflow-hidden`}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20">
          ...
          <div className="flex items-center gap-2 text-white/70 text-xs font-semibold mb-4 flex-wrap">
            ...
            <Link href={`/courses/${NATURAL_TOXINS_META.semesterSlug}`} className="hover:text-white transition">
              {NATURAL_TOXINS_META.semester}
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white">Natural Toxins</span>
          </div>
          ...
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 border border-white/30 text-white text-xs font-bold uppercase tracking-widest mb-5">
            <Zap className="w-3.5 h-3.5" /> {NATURAL_TOXINS_META.semester} · {NATURAL_TOXINS_META.subjectCode}
          </span>

          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white mb-2 tracking-tight leading-tight">
            Natural
            <span className="block text-emerald-200 mt-1">Toxins</span>
          </h1>
          <p className="text-white/80 text-sm sm:text-base max-w-2xl mb-8">
            {NATURAL_TOXINS_META.description}
          </p>

          <div className="flex flex-wrap gap-3 mb-8">
            <Link href={`${BASE_PATH}/${NaturalToxinsUnits[0].id}`}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-green-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300">
          ...
          <div className="flex flex-wrap gap-6 sm:gap-10">
            {[
              { n: String(NaturalToxinsUnits.length),                                             l: "Units"  },
              { n: String(NaturalToxinsUnits.filter(u => u.difficulty === "Easy").length),   l: "Easy"   },
              { n: String(NaturalToxinsUnits.filter(u => u.difficulty === "Medium").length), l: "Medium" },
              { n: String(NaturalToxinsUnits.filter(u => u.difficulty === "Hard").length),   l: "Hard"   },
            ].map(({ n, l }) => (
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search units…" value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-11 pr-10 py-3 rounded-2xl border-2 border-gray-200 focus:border-green-400 focus:outline-none text-sm text-gray-800 placeholder:text-gray-400 bg-white transition-all" />
            {search && (
            ...
            {diffs.map(d => (
              <button key={d} onClick={() => setActiveDiff(d)}
                className={`shrink-0 px-4 py-2 rounded-xl text-xs font-extrabold uppercase tracking-wide transition-all duration-200 ${
                  activeDiff === d ? `bg-gradient-to-r ${HERO_GRAD} text-white shadow-md` : "bg-gray-100 text-gray-500 hover:bg-gray-200"
                }`}>{d}</button>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {filtered.map((unit) => (
              <Link key={unit.id} href={`${BASE_PATH}/${unit.id}`}
                className="group relative flex flex-col rounded-2xl border border-gray-200 bg-white overflow-hidden hover:border-green-300 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 h-full">
                ...
                <div className="flex flex-col flex-1 p-4 sm:p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${NATURAL_TOXINS_DIFF_BADGE[unit.difficulty]}`}>
                      {unit.difficulty}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className={`text-[10px] font-extrabold uppercase tracking-wide ${unit.color.iconText}`}>
                      Unit {NaturalToxinsUnits.indexOf(unit) + 1} of {NaturalToxinsUnits.length}
                    </span>
                  </div>
                  <h3 className="font-extrabold text-gray-900 text-sm sm:text-base mb-2 leading-snug group-hover:text-green-700 transition-colors">
                    {unit.shortTitle}
                  </h3>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-5">
            <div>
              ...
              <p className="text-white/80 text-xs sm:text-sm">
                Start with <span className="font-bold text-white">Unit 1 — Introduction to Natural Toxins</span> and work your way up.
              </p>
            </div>
            <Link href={`${BASE_PATH}/${NaturalToxinsUnits[0].id}`}
              className="inline-flex items-center gap-2 px-5 sm:px-6 py-3 rounded-xl bg-white text-green-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 shrink-0">
