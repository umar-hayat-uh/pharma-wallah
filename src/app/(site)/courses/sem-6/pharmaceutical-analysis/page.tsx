...
// app/courses/sem-6/pharmaceutical-analysis/page.tsx
...
import { PharmaAnalysisUnits, PHARMA_ANALYSIS_META, PHARMA_ANALYSIS_DIFF_BADGE } from "@/app/api/pharmaceutical-analysis-data";
...
const HERO_GRAD = "from-cyan-600 to-teal-500";
const BASE_PATH = `/courses/${PHARMA_ANALYSIS_META.semesterSlug}/${PHARMA_ANALYSIS_META.slug}`;

export default function PharmaceuticalAnalysisUnitsPage() {
  const [search,     setSearch]     = useState("");
  const [activeDiff, setActiveDiff] = useState("All");
  const diffs = ["All", "Easy", "Medium", "Hard"];

  const filtered = useMemo(() =>
    PharmaAnalysisUnits.filter(u =>
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      {bgIcons.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-cyan-100 z-0 hidden md:block" style={{ top, left }}>
      ...
      <div className={`relative bg-gradient-to-r ${HERO_GRAD} overflow-hidden`}>
        ...
        <div className="absolute right-10 bottom-6 opacity-[0.10] pointer-events-none hidden sm:block">
          <Beaker size={100} className="text-white" />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20">
          ...
          <div className="flex items-center gap-2 text-white/70 text-xs font-semibold mb-4 flex-wrap">
            ...
            <Link href={`/courses/${PHARMA_ANALYSIS_META.semesterSlug}`} className="hover:text-white transition">
              {PHARMA_ANALYSIS_META.semester}
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white">Pharmaceutical Analysis</span>
          </div>
          ...
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 border border-white/30 text-white text-xs font-bold uppercase tracking-widest mb-5">
            <Zap className="w-3.5 h-3.5" /> {PHARMA_ANALYSIS_META.semester} · {PHARMA_ANALYSIS_META.subjectCode}
          </span>

          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white mb-2 tracking-tight leading-tight">
            Pharmaceutical
            <span className="block text-teal-200 mt-1">Analysis</span>
          </h1>
          <p className="text-white/80 text-sm sm:text-base max-w-2xl mb-8">
            {PHARMA_ANALYSIS_META.description}
          </p>

          <div className="flex flex-wrap gap-3 mb-8">
            <Link href={`${BASE_PATH}/${PharmaAnalysisUnits[0].id}`}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-cyan-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300">
          ...
          <div className="flex flex-wrap gap-6 sm:gap-10">
            {[
              { n: String(PharmaAnalysisUnits.length),                                             l: "Units"  },
              { n: String(PharmaAnalysisUnits.filter(u => u.difficulty === "Easy").length),   l: "Easy"   },
              { n: String(PharmaAnalysisUnits.filter(u => u.difficulty === "Medium").length), l: "Medium" },
              { n: String(PharmaAnalysisUnits.filter(u => u.difficulty === "Hard").length),   l: "Hard"   },
            ].map(({ n, l }) => (
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search units…" value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-11 pr-10 py-3 rounded-2xl border-2 border-gray-200 focus:border-cyan-400 focus:outline-none text-sm text-gray-800 placeholder:text-gray-400 bg-white transition-all" />
            {search && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {filtered.map((unit) => (
              <Link key={unit.id} href={`${BASE_PATH}/${unit.id}`}
                className="group relative flex flex-col rounded-2xl border border-gray-200 bg-white overflow-hidden hover:border-cyan-300 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 h-full">
                ...
                <div className="flex flex-col flex-1 p-4 sm:p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${PHARMA_ANALYSIS_DIFF_BADGE[unit.difficulty]}`}>
                      {unit.difficulty}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className={`text-[10px] font-extrabold uppercase tracking-wide ${unit.color.iconText}`}>
                      Unit {PharmaAnalysisUnits.indexOf(unit) + 1} of {PharmaAnalysisUnits.length}
                    </span>
                  </div>
                  <h3 className="font-extrabold text-gray-900 text-sm sm:text-base mb-2 leading-snug group-hover:text-cyan-700 transition-colors">
                    {unit.shortTitle}
                  </h3>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-5">
            <div>
              ...
              <p className="text-white/80 text-xs sm:text-sm">
                Start with <span className="font-bold text-white">Unit 1 — Chemical Methods</span> and work your way up.
              </p>
            </div>
            <Link href={`${BASE_PATH}/${PharmaAnalysisUnits[0].id}`}
              className="inline-flex items-center gap-2 px-5 sm:px-6 py-3 rounded-xl bg-white text-cyan-700 font-extrabold text-sm shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 shrink-0">
