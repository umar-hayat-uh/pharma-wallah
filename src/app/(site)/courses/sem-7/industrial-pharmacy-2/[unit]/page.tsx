"use client";
// IndustrialPharmacy2UnitPage.tsx
...
import { IND_PHARM2_META, IndPharm2Units } from "@/app/api/industrial-pharmacy-2-data";
...
const mdComponents = {
  ...
  h3: ({ children }: any) => <h3 className="text-base sm:text-lg font-bold text-blue-700 mt-6 mb-3 break-words">{children}</h3>,
  ...
  blockquote: ({ children }: any) => <blockquote className="border-l-4 border-blue-400 bg-blue-50/60 px-4 py-3 rounded-r-xl my-4 text-gray-700 text-sm sm:text-base italic break-words">{children}</blockquote>,
  ...
  tr:     ({ children }: any) => <tr className="hover:bg-blue-50/20 transition-colors">{children}</tr>,
  code:   ({ inline, children }: any) =>
    inline
      ? <code className="bg-gray-100 text-blue-700 px-1.5 py-0.5 rounded text-xs font-mono break-all">{children}</code>
      : <pre className="bg-gray-900 text-blue-300 rounded-2xl p-4 overflow-x-auto text-xs font-mono my-4 leading-relaxed"><code>{children}</code></pre>,
  hr: () => <hr className="my-8 border-gray-200" />,
};

export default function UnitPage({ params }: PageProps) {
  const { unit: unitSlug } = params;
  const unitIdx  = IndPharm2Units.findIndex(u => u.id === unitSlug);
  const unit     = IndPharm2Units[unitIdx];
  const prevUnit = unitIdx > 0 ? IndPharm2Units[unitIdx - 1] : null;
  const nextUnit = unitIdx < IndPharm2Units.length - 1 ? IndPharm2Units[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/${IND_PHARM2_META.slug}/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      const addHeader = (doc: typeof pdf, p: number, t: number) => {
        ...
        doc.text(`PHARMAWALLAH  ·  ${IND_PHARM2_META.title.toUpperCase()}`, margin, 6.5);
  ...
  if (!unit) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <Link href={BASE_PATH} className="text-blue-600 text-sm font-semibold hover:underline">← Back to Industrial Pharm II</Link>
      </div>
    </div>
  );
    ...
    <section className="min-h-screen bg-white relative" style={{ overflowX: "hidden" }}>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-blue-600 shrink-0 font-medium">Industrial Pharm II</Link>
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[{ href: "/", label: "Home" }, { href: "/courses", label: "Courses" }, { href: `/courses/${IND_PHARM2_META.semesterSlug}`, label: IND_PHARM2_META.semester }, { href: BASE_PATH, label: IND_PHARM2_META.title }].map(({ href, label }) => (
          ...
          <aside className="hidden lg:block w-52 xl:w-60 flex-shrink-0">
            <div className="sticky top-6 space-y-4">
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1"><Zap size={11} className="opacity-80" /><span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Industrial Pharm II</span></div>
                <p className="font-bold text-xs leading-snug mt-1">{IND_PHARM2_META.semester} · {IND_PHARM2_META.subjectCode}</p>
              </div>
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Industrial Pharm II · {IND_PHARM2_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Chapter ${unitIdx + 1} of ${IndPharm2Units.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
                ...
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-blue-600 font-mono">public/content/${IND_PHARM2_META.slug}/${unitSlug}.md</code>
                    </p>
