...
import { HospPharmUnits, HOSP_PHARM_META } from "@/app/api/hospital-pharmacy-data";
...
const BASE_PATH = `/courses/${HOSP_PHARM_META.semesterSlug}/${HOSP_PHARM_META.slug}`;
...
export default function UnitPage({ params }: PageProps) {
  ...
  const unitIdx  = HospPharmUnits.findIndex(u => u.id === unitSlug);
  const unit     = HospPharmUnits[unitIdx];
  const prevUnit = unitIdx > 0 ? HospPharmUnits[unitIdx - 1] : null;
  const nextUnit = unitIdx < HospPharmUnits.length - 1 ? HospPharmUnits[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/${HOSP_PHARM_META.slug}/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      const addHeader = (doc: typeof pdf, p: number, t: number) => {
        ...
        doc.text(`PHARMAWALLAH  ·  ${HOSP_PHARM_META.title.toUpperCase()}`, margin, 6.5);
  ...
  if (!unit) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <Link href={BASE_PATH} className="text-blue-600 text-sm font-semibold hover:underline">← Back to Hospital Pharmacy</Link>
      </div>
    </div>
  );
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-blue-600 shrink-0 font-medium">Hospital Pharmacy</Link>
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[
            ...
            { href: `/courses/${HOSP_PHARM_META.semesterSlug}`, label: HOSP_PHARM_META.semester },
            { href: BASE_PATH, label: HOSP_PHARM_META.title },
          ].map(({ href, label }) => (
            ...
            <div className="sticky top-6 space-y-4">
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Hospital Pharmacy</span>
                </div>
                <p className="font-bold text-xs leading-snug mt-1">{HOSP_PHARM_META.semester} · {HOSP_PHARM_META.subjectCode}</p>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3 space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Clock size={12} className="text-blue-500 shrink-0" /><span>{unit.readTime} min read</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <GraduationCap size={12} className="text-blue-500 shrink-0" /><span>Difficulty: <strong>{unit.difficulty}</strong></span>
                </div>
              </div>
              <button onClick={handleDownloadPdf} disabled={pdfLoading}
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Hospital Pharmacy · {HOSP_PHARM_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Chapter ${unitIdx + 1} of ${HospPharmUnits.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
                ...
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-blue-600 font-mono">
                        public/content/${HOSP_PHARM_META.slug}/${unitSlug}.md
                      </code>
                    </p>
