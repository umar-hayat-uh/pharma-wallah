...
import {
  ...
  Clock, Tag, Zap, Layers, GraduationCap, ArrowRight, Building2,
} from "lucide-react";
import { HospPharmUnits, HOSP_PHARM_META, HOSP_PHARM_DIFF_BADGE } from "@/app/api/hospital-pharmacy-data";
...
const HERO_GRAD = "from-blue-700 to-teal-500";
    ...
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      ...
      <div className={`relative bg-gradient-to-r ${HERO_GRAD} overflow-hidden`}>
        ...
        <div className="absolute right-10 bottom-6 opacity-[0.10] pointer-events-none hidden sm:block">
          <Building2 size={110} className="text-white" />
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20">
          <div className="flex items-center gap-2 text-white/70 text-xs font-semibold mb-4 flex-wrap">
            ...
            <Link href={`/courses/${HOSP_PHARM_META.semesterSlug}`} className="hover:text-white transition">{HOSP_PHARM_META.semester}</Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white">Hospital Pharmacy</span>
          </div>
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 border border-white/30 text-white text-xs font-bold uppercase tracking-widest mb-5">
            <Zap className="w-3.5 h-3.5" /> {HOSP_PHARM_META.semester} · {HOSP_PHARM_META.subjectCode}
          </span>
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white mb-2 tracking-tight leading-tight">
            {HOSP_PHARM_META.title.split(" ").slice(0, -1).join(" ")}
            <span className="block text-white/70 mt-1">{HOSP_PHARM_META.title.split(" ").slice(-1)[0]}</span>
          </h1>
          <p className="text-white/80 text-sm sm:text-base max-w-2xl mb-8">{HOSP_PHARM_META.description}</p>
          <div className="flex flex-wrap gap-3 mb-8">
            <Link href={`${BASE_PATH}/${HospPharmUnits[0].id}`}
          ...
          <div className="flex flex-wrap gap-6 sm:gap-10">
            {[
              { n: String(HospPharmUnits.length), l: "Units" },
              { n: String(HospPharmUnits.filter(u => u.difficulty === "Easy").length),   l: "Easy"   },
              { n: String(HospPharmUnits.filter(u => u.difficulty === "Medium").length), l: "Medium" },
              { n: String(HospPharmUnits.filter(u => u.difficulty === "Hard").length),   l: "Hard"   },
            ].map(({ n, l }) => (
              ...
              <Link key={unit.id} href={`${BASE_PATH}/${unit.id}`}
                ...
                <div className="flex flex-col flex-1 p-4 sm:p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${HOSP_PHARM_DIFF_BADGE[unit.difficulty]}`}>{unit.difficulty}</span>
                  </div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className={`text-[10px] font-extrabold uppercase tracking-wide ${unit.color.iconText}`}>
                      {HospPharmUnits.indexOf(unit) + 1} of {HospPharmUnits.length}
                    </span>
                  </div>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-5">
            <div>
              ...
              <p className="text-white/80 text-xs sm:text-sm">
                Start with <span className="font-bold text-white">Chapter 1 — {HospPharmUnits[0].shortTitle}</span>
              </p>
            </div>
            <Link href={`${BASE_PATH}/${HospPharmUnits[0].id}`}
