"use client";
// PharmaceuticalTechnologyUnitsPage.tsx
...
import { PHARMATECH_META, PharmaTechUnits, PHARMATECH_DIFF_BADGE } from "@/app/api/pharmaceutical-technology-data";
...
const HERO_GRAD = "from-violet-700 to-rose-500";
  ...
  return (
    <section className="min-h-screen bg-white relative overflow-x-hidden">
      {bgIcons.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-violet-100 z-0 hidden md:block" style={{ top, left }}>
      ...
      <div className={`relative bg-gradient-to-r ${HERO_GRAD} overflow-hidden`}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20">
          <div className="flex items-center gap-2 text-white/70 text-xs font-semibold mb-4 flex-wrap">
            ...
            <Link href={`/courses/${PHARMATECH_META.semesterSlug}`} className="hover:text-white transition">{PHARMATECH_META.semester}</Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white">Pharmaceutical Technology</span>
          </div>
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 border border-white/30 text-white text-xs font-bold uppercase tracking-widest mb-5">
            <Zap className="w-3.5 h-3.5" /> {PHARMATECH_META.semester} · {PHARMATECH_META.subjectCode}
          </span>
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white mb-2 tracking-tight leading-tight">
            {PHARMATECH_META.title.split(" ").slice(0, -1).join(" ")}
            <span className="block text-white/70 mt-1">{PHARMATECH_META.title.split(" ").slice(-1)[0]}</span>
          </h1>
          <p className="text-white/80 text-sm sm:text-base max-w-2xl mb-8">{PHARMATECH_META.description}</p>
          <div className="flex flex-wrap gap-3 mb-8">
            <Link href={`${BASE_PATH}/${PharmaTechUnits[0].id}`}
          ...
          <div className="flex flex-wrap gap-6 sm:gap-10">
            {[
              { n: String(PharmaTechUnits.length), l: "Chapters" },
              { n: String(PharmaTechUnits.filter(u => u.difficulty === "Easy").length),   l: "Easy"   },
              { n: String(PharmaTechUnits.filter(u => u.difficulty === "Medium").length), l: "Medium" },
              { n: String(PharmaTechUnits.filter(u => u.difficulty === "Hard").length),   l: "Hard"   },
            ].map(({ n, l }) => (
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search chapters…" value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-11 pr-10 py-3 rounded-2xl border-2 border-gray-200 focus:border-violet-400 focus:outline-none text-sm text-gray-800 placeholder:text-gray-400 bg-white transition-all" />
            {search && (
              ...
              <Link key={unit.id} href={`${BASE_PATH}/${unit.id}`}
                ...
                <div className="flex flex-col flex-1 p-4 sm:p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${PHARMATECH_DIFF_BADGE[unit.difficulty]}`}>{unit.difficulty}</span>
                  </div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className={`text-[10px] font-extrabold uppercase tracking-wide ${unit.color.iconText}`}>
                      {PharmaTechUnits.indexOf(unit) + 1} of {PharmaTechUnits.length}
                    </span>
                  </div>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-5">
            <div>
              ...
              <p className="text-white/80 text-xs sm:text-sm">
                Start with <span className="font-bold text-white">Chapter 1 — {PharmaTechUnits[0].shortTitle}</span>
              </p>
            </div>
            <Link href={`${BASE_PATH}/${PharmaTechUnits[0].id}`}
