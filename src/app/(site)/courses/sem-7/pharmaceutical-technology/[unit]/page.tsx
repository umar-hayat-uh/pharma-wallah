"use client";
// PharmaceuticalTechnologyUnitPage.tsx
...
import { PHARMATECH_META, PharmaTechUnits } from "@/app/api/pharmaceutical-technology-data";
...
const BASE_PATH = `/courses/${PHARMATECH_META.semesterSlug}/${PHARMATECH_META.slug}`;
...
const mdComponents = {
  h1: ({ children }: any) => <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-gray-900 mt-8 mb-4 pb-3 border-b-2 border-gray-100 break-words">{children}</h1>,
  ...
  h3: ({ children }: any) => <h3 className="text-base sm:text-lg font-bold text-violet-700 mt-6 mb-3 break-words">{children}</h3>,
  h4: ({ children }: any) => <h4 className="text-sm sm:text-base font-semibold text-gray-700 mt-5 mb-2 break-words">{children}</h4>,
  ...
  li: ({ children }: any) => (
    <li className="flex gap-2.5 text-gray-700 text-sm sm:text-base leading-relaxed">
      <span className="mt-[7px] w-1.5 h-1.5 rounded-full bg-violet-500 flex-shrink-0" />
      <span className="break-words min-w-0 flex-1">{children}</span>
    </li>
  ),
  strong:     ({ children }: any) => <strong className="font-bold text-gray-900">{children}</strong>,
  em:         ({ children }: any) => <em className="italic text-gray-700">{children}</em>,
  blockquote: ({ children }: any) => <blockquote className="border-l-4 border-violet-400 bg-violet-50/60 px-4 py-3 rounded-r-xl my-4 text-gray-700 text-sm sm:text-base italic break-words">{children}</blockquote>,
  table:  ({ children }: any) => <ScrollTable>{children}</ScrollTable>,
  thead:  ({ children }: any) => <thead className="bg-violet-50">{children}</thead>,
  ...
  tr:     ({ children }: any) => <tr className="hover:bg-violet-50/20 transition-colors">{children}</tr>,
  code:   ({ inline, children }: any) =>
    inline
      ? <code className="bg-gray-100 text-violet-700 px-1.5 py-0.5 rounded text-xs font-mono break-all">{children}</code>
...
export default function UnitPage({ params }: PageProps) {
  ...
  const unitIdx  = PharmaTechUnits.findIndex(u => u.id === unitSlug);
  const unit     = PharmaTechUnits[unitIdx];
  const prevUnit = unitIdx > 0 ? PharmaTechUnits[unitIdx - 1] : null;
  const nextUnit = unitIdx < PharmaTechUnits.length - 1 ? PharmaTechUnits[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/${PHARMATECH_META.slug}/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      const margin = 15; const headerSpace = 12;
      ...
      const addHeader = (doc: typeof pdf, p: number, t: number) => {
        doc.setFillColor(r, g, b); doc.rect(0, 0, pageW, 10, "F");
        doc.setTextColor(255, 255, 255); doc.setFontSize(7);
        doc.setFont("helvetica", "bold");
        doc.text(`PHARMAWALLAH  ·  ${PHARMATECH_META.title.toUpperCase()}`, margin, 6.5);
      ...
      for (let page = 0; page < totalPages; page++) {
        ...
        sc.width = canvas.width; sc.height = slicePxH;
  ...
  if (!unit) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="text-center">
        <p className="text-xl font-extrabold text-gray-900 mb-2">Chapter not found</p>
        <Link href={BASE_PATH} className="text-violet-600 text-sm font-semibold hover:underline">← Back to Pharma Technology</Link>
      </div>
    </div>
  );
  ...
  return (
    <section className="min-h-screen bg-white relative" style={{ overflowX: "hidden" }}>
      {bgIconDefs.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-violet-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-violet-600 shrink-0 font-medium">Pharma Technology</Link>
            <ChevronRight size={11} className="shrink-0 mx-0.5 text-gray-300" />
            <span className="text-violet-700 font-semibold truncate">{unit.shortTitle}</span>
          </div>
        ...
        {mobileNavOpen && (
          <div className="bg-white border-t border-gray-100 px-3 pb-3 shadow-xl max-h-64 overflow-y-auto">
            <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400 pt-2 mb-1.5">All Chapters</p>
            <ul className="space-y-0.5">
              {PharmaTechUnits.map(u => (
                <li key={u.id}>
                  <Link href={`${BASE_PATH}/${u.id}`} onClick={() => setMobileNavOpen(false)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs transition-all ${u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold` : "text-gray-700 hover:bg-gray-50"}`}>
        ...
        <div className="flex border-t border-gray-100">
          {prevUnit ? (
            <Link href={`${BASE_PATH}/${prevUnit.id}`} className="flex-1 flex items-center gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50 transition-colors min-w-0">
              <ChevronLeft size={13} className="shrink-0" /><span className="truncate">{prevUnit.emoji} {prevUnit.shortTitle}</span>
            </Link>
          ) : <div className="flex-1" />}
          {nextUnit && (
            <Link href={`${BASE_PATH}/${nextUnit.id}`} className="flex-1 flex items-center justify-end gap-1.5 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50 transition-colors border-l border-gray-100 min-w-0">
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[{ href: "/", label: "Home" }, { href: "/courses", label: "Courses" }, { href: `/courses/${PHARMATECH_META.semesterSlug}`, label: PHARMATECH_META.semester }, { href: BASE_PATH, label: PHARMATECH_META.title }].map(({ href, label }) => (
            <span key={href} className="flex items-center gap-1.5">
              <Link href={href} className="hover:text-violet-600 transition-colors">{label}</Link>
              <ChevronRight size={13} className="text-gray-300" />
            </span>
          ))}
          <span className="text-violet-700 font-semibold">{unit.shortTitle}</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-8 items-start w-full">
          ...
          <aside className="hidden lg:block w-52 xl:w-60 flex-shrink-0">
            <div className="sticky top-6 space-y-4">
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1"><Zap size={11} className="opacity-80" /><span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Pharma Technology</span></div>
                <p className="font-bold text-xs leading-snug mt-1">{PHARMATECH_META.semester} · {PHARMATECH_META.subjectCode}</p>
              </div>
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3">
                <p className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2"><BookOpen size={11} /> All Chapters</p>
                <ul className="space-y-0.5">
                  {PharmaTechUnits.map(u => (
                    <li key={u.id}>
                      <Link href={`${BASE_PATH}/${u.id}`}
                        className={`group flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs transition-all ${u.id === unitSlug ? `bg-gradient-to-r ${GRAD} text-white font-semibold shadow-sm` : "text-gray-600 hover:bg-gray-50"}`}>
                        <span>{u.emoji}</span>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3 space-y-2">
                <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2">Chapter Info</p>
                <div className="flex items-center gap-2 text-xs text-gray-600"><Clock size={12} className="text-violet-500 shrink-0" /><span>{unit.readTime} min read</span></div>
                <div className="flex items-center gap-2 text-xs text-gray-600"><GraduationCap size={12} className="text-violet-500 shrink-0" /><span>Difficulty: <strong>{unit.difficulty}</strong></span></div>
              </div>
              <button onClick={handleDownloadPdf} disabled={pdfLoading}
          ...
          <div className="flex-1 min-w-0 w-full">
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="absolute bottom-3 right-3 opacity-10 pointer-events-none select-none"><span className="text-6xl sm:text-8xl">{unit.emoji}</span></div>
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Pharma Technology · {PHARMATECH_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Chapter ${unitIdx + 1} of ${PharmaTechUnits.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
            ...
            <div ref={printRef} style={{ background: "#fff" }}>
              {unit.previewImage && !imgError && (
                <div className="mb-4 sm:mb-5 rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-gray-50">
                  <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-100 bg-white">
                    <ImageIcon size={13} className="text-violet-500 shrink-0" />
                    <span className="text-xs font-semibold text-gray-500">Chapter Overview</span>
                  </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3 sm:p-5 md:p-8 overflow-hidden" style={{ wordBreak: "break-word", overflowWrap: "break-word" }}>
                {loading && (
                  <div className="py-16 flex flex-col items-center gap-4">
                    <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${GRAD} flex items-center justify-center animate-pulse`}><BookOpen className="w-5 h-5 text-white" /></div>
                    <p className="text-gray-400 text-sm font-semibold">Loading chapter content…</p>
                  </div>
                )}
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    <div className="w-12 h-12 rounded-2xl bg-red-50 border border-red-200 flex items-center justify-center mx-auto mb-4"><BookOpen className="w-5 h-5 text-red-400" /></div>
                    <p className="text-gray-700 font-bold mb-1">Content file not found</p>
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-violet-600 font-mono">public/content/${PHARMATECH_META.slug}/${unitSlug}.md</code>
                    </p>
                    <Link href={BASE_PATH} className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r ${GRAD} text-white text-xs font-extrabold`}>← Back to all chapters</Link>
                  </div>
                )}
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              {prevUnit ? (
                <Link href={`${BASE_PATH}/${prevUnit.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center flex-shrink-0"><ChevronLeft size={15} className="text-gray-500" /></div>
              ...
              {nextUnit ? (
                <Link href={`${BASE_PATH}/${nextUnit.id}`}
                  ...
                  <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center flex-shrink-0"><ChevronRight size={15} className="text-gray-500" /></div>
                </Link>
              ) : <div />}
            </div>
          </div>
