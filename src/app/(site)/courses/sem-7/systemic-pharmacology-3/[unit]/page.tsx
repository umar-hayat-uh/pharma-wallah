"use client";
// SystemicPharmacology3UnitPage.tsx
...
import { SysPharm3Units, SYS_PHARM3_META } from "@/app/api/systemic-pharmacology-3-data";
...
const BASE_PATH = `/courses/${SYS_PHARM3_META.semesterSlug}/${SYS_PHARM3_META.slug}`;
...
const mdComponents = {
  ...
  h3: ({ children }: any) => <h3 className="text-base sm:text-lg font-bold text-indigo-700 mt-6 mb-3 break-words">{children}</h3>,
  ...
  li: ({ children }: any) => (
    <li className="flex gap-2.5 text-gray-700 text-sm sm:text-base leading-relaxed">
      <span className="mt-[7px] w-1.5 h-1.5 rounded-full bg-indigo-500 flex-shrink-0" />
      <span className="break-words min-w-0 flex-1">{children}</span>
    </li>
  ),
  strong:     ({ children }: any) => <strong className="font-bold text-gray-900">{children}</strong>,
  em:         ({ children }: any) => <em className="italic text-gray-700">{children}</em>,
  blockquote: ({ children }: any) => <blockquote className="border-l-4 border-indigo-400 bg-indigo-50/60 px-4 py-3 rounded-r-xl my-4 text-gray-700 text-sm sm:text-base italic break-words">{children}</blockquote>,
  ...
  tr:     ({ children }: any) => <tr className="hover:bg-indigo-50/20 transition-colors">{children}</tr>,
  code:   ({ inline, children }: any) =>
    inline
      ? <code className="bg-gray-100 text-indigo-700 px-1.5 py-0.5 rounded text-xs font-mono break-all">{children}</code>
      : <pre className="bg-gray-900 text-indigo-300 rounded-2xl p-4 overflow-x-auto text-xs font-mono my-4 leading-relaxed"><code>{children}</code></pre>,
  hr: () => <hr className="my-8 border-gray-200" />,
};

export default function UnitPage({ params }: PageProps) {
  const { unit: unitSlug } = params;
  const unitIdx  = SysPharm3Units.findIndex(u => u.id === unitSlug);
  const unit     = SysPharm3Units[unitIdx];
  const prevUnit = unitIdx > 0 ? SysPharm3Units[unitIdx - 1] : null;
  const nextUnit = unitIdx < SysPharm3Units.length - 1 ? SysPharm3Units[unitIdx + 1] : null;
  ...
  useEffect(() => {
    ...
    fetch(`/content/${SYS_PHARM3_META.slug}/${unitSlug}.md`)
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      const addHeader = (doc: typeof pdf, p: number, t: number) => {
        const [r, g, b] = [67,56,202];
        doc.setFillColor(r, g, b); doc.rect(0, 0, pageW, 10, "F");
        doc.setTextColor(255, 255, 255); doc.setFontSize(7);
        doc.setFont("helvetica", "bold");
        doc.text(`PHARMAWALLAH  ·  ${SYS_PHARM3_META.title.toUpperCase()}`, margin, 6.5);
  ...
  if (!unit) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <Link href={BASE_PATH} className="text-indigo-600 text-sm font-semibold hover:underline">← Back to Systemic Pharm III</Link>
      </div>
    </div>
  );
  ...
  return (
    <section className="min-h-screen bg-white relative" style={{ overflowX: "hidden" }}>
      {bgIconDefs.map(({ Icon, top, left, size }, i) => (
        <div key={i} className="fixed pointer-events-none text-indigo-100 z-0 hidden sm:block" style={{ top, left }}>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-3 py-2.5">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link href={BASE_PATH} className="hover:text-indigo-600 shrink-0 font-medium">Systemic Pharm III</Link>
            <ChevronRight size={11} className="shrink-0 mx-0.5 text-gray-300" />
            <span className="text-indigo-700 font-semibold truncate">{unit.shortTitle}</span>
          </div>
      ...
      <div className="relative z-10 mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 py-4 sm:py-6 lg:py-10 w-full">
        <nav className="hidden lg:flex items-center gap-1.5 text-sm text-gray-500 mb-5 flex-wrap">
          {[
            ...
            { href: `/courses/${SYS_PHARM3_META.semesterSlug}`, label: SYS_PHARM3_META.semester },
            { href: BASE_PATH, label: SYS_PHARM3_META.title },
          ].map(({ href, label }) => (
          ...
          <span className="text-indigo-700 font-semibold">{unit.shortTitle}</span>
        </nav>
            ...
            <div className="sticky top-6 space-y-4">
              <div className={`bg-gradient-to-br ${GRAD} rounded-2xl p-4 text-white shadow-lg`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-80">Systemic Pharm III</span>
                </div>
                <p className="font-bold text-xs leading-snug mt-1">{SYS_PHARM3_META.semester} · {SYS_PHARM3_META.subjectCode}</p>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3 space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Clock size={12} className="text-indigo-500 shrink-0" /><span>{unit.readTime} min read</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <GraduationCap size={12} className="text-indigo-500 shrink-0" /><span>Difficulty: <strong>{unit.difficulty}</strong></span>
                </div>
              </div>
              <button onClick={handleDownloadPdf} disabled={pdfLoading}
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest mb-2.5">
                  <Zap size={9} /> Systemic Pharm III · {SYS_PHARM3_META.semester}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Chapter ${unitIdx + 1} of ${SysPharm3Units.length}`, `${unit.readTime} min`, unit.difficulty].map(t => (
                ...
                {error && !loading && (
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-indigo-600 font-mono">
                        public/content/${SYS_PHARM3_META.slug}/${unitSlug}.md
                      </code>
                    </p>
