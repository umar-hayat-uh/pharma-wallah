...
export const HISTOLOGY_LESSONS: NavLink[] = [
  ...
  { id: "wbcs", title: "WBC's" },
  { id: "connective-tissue", title: "Connective Tissue" },
  { id: "epithelium-simple", title: "Simple  Epithelium" },
  { id: "epithelium-stratified", title: "Stratified Epithelium" },
  { id: "slide-preparation", title: "Slide Preparation & Staining" },
  { id: "gall-bladder-skin", title: "Gall Bladder " },
  { id: "cardiac-muscle", title: "Cardiac Muscle" },
];
    ...
    <div className="relative rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm">
      ...
      <div className="p-4 sm:p-6">
          ...
          {refs.map((ref, i) => (
            <li key={i} className="flex gap-2.5">
              ...
              <p className="text-xs text-gray-700 leading-relaxed break-words w-full">
                ...
                <em className="font-semibold text-gray-900 not-italic">
                  {ref.title}
                </em>
                {ref.edition && (
                  <span className="text-gray-400"> ({ref.edition})</span>
                )}
                ...
                {ref.url && (
                  ...
                  >
                    ...
                    {ref.url.replace("https://", "").replace("http://", "")}
                  </a>
                )}
              </p>
            </li>
        ...
        <p className="text-[10px] text-gray-400 mt-4 pt-3 border-t border-gray-100 leading-relaxed">
          <strong className="text-gray-500">Disclaimer:</strong> These notes are
          for educational purposes only and compiled from standard histology
          textbooks. Clinical interpretation of slides requires a qualified
  ...
  return (
    ...
    >
      ...
      {BG_ICONS.map(({ Icon, top, left, size }, i) => (
        <div
          key={i}
          className="fixed pointer-events-none text-blue-100 z-0 hidden sm:block"
          style={{ top, left }}
        >
      ...
      {/* ── Mobile top bar – matches pathology design ── */}
      <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        ...
        <div className="flex items-center justify-between px-3 py-2.5 pt-[2.625rem]">
          <div className="flex items-center gap-1 text-xs text-gray-500 min-w-0 flex-1 mr-2">
            <Link
              href="/spotting/histology/lessons"
        ...
        {mobileNavOpen && (
          <div className="bg-white border-t border-gray-100 px-3 pb-3 shadow-xl max-h-[60vh] overflow-y-auto">
            <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400 pt-2 mb-1.5">
              All Lessons
            </p>
              ...
              {HISTOLOGY_LESSONS.map((l) => (
                <li key={l.id}>
                  ...
                  >
                    ...
                    {l.id === id && (
                      <span className="text-[9px] bg-white/25 px-1.5 py-0.5 rounded-full shrink-0">
                        Now
                      </span>
                    )}
                  </Link>
      ...
      {/* ── Page Layout – minimal top padding on mobile (pt-4) ── */}
        ...
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-8 items-start w-full">
                  ...
                  {HISTOLOGY_LESSONS.map((l) => (
                    <li key={l.id}>
                      ...
                      >
                        ...
                        <ChevronRight
                          size={11}
                          className={`shrink-0 transition-opacity ${l.id === id ? "opacity-100" : "opacity-0 group-hover:opacity-60"}`}
                        />
                      </Link>
                    </li>
                  ))}
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-3">
                ...
                <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${((lessonIdx + 1) / HISTOLOGY_LESSONS.length) * 100}%`,
                    }}
                  />
                </div>
                <p className="text-[10px] text-gray-400 mt-1.5">
                  Lesson {lessonIdx + 1} of {HISTOLOGY_LESSONS.length}
                </p>
              </div>

              {/* Take test */}
              <Link
                href="/spotting/histology/test"
          ...
          <div className="flex-1 min-w-0 w-full space-y-5">
            ...
            >
              ...
              <div className="absolute bottom-3 right-4 opacity-10 pointer-events-none select-none text-6xl sm:text-8xl">
                {emoji}
              </div>
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                ...
                <p className="text-white/80 text-xs sm:text-sm">
                  Lesson {lessonIdx + 1} of {HISTOLOGY_LESSONS.length} ·
                  Detailed theory + identification points
                </p>
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[
                    "Histology",
                    category,
                    `${pointsOfIdentification.length} ID Points`,
                  ].map((t) => (
                    <span
                      key={t}
            ...
            <div className="relative rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm">
              ...
              <div className="p-4 sm:p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div
                    className={`w-8 h-8 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center shrink-0`}
                  >
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {pointsOfIdentification.map((point, i) => (
                    ...
                    >
                      <span
                        className={`w-6 h-6 rounded-lg bg-gradient-to-br ${gradient} flex items-center justify-center text-white text-[10px] font-extrabold shrink-0 mt-0.5`}
                      >
                        {i + 1}
                      </span>
            ...
            {/* Take test CTA */}
            <div
              className={`relative rounded-2xl bg-gradient-to-r ${gradient} overflow-hidden p-5 sm:p-7`}
            >
              <div className="absolute -top-8 -right-8 w-36 h-36 rounded-full bg-white/10 pointer-events-none" />
              <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  <p className="text-white font-extrabold text-sm sm:text-base mb-0.5">
