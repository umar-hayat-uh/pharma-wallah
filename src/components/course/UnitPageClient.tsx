...
// src/components/course/UnitPageClient.tsx
// Receives already-fetched markdown as a prop from the server component —
// it does NOT fetch anything itself. It only owns the bits that truly need
// the browser: the print ref, scroll-to-top visibility, and image fallback.
...
export default function UnitPageClient({ subject, unit, prevUnit, nextUnit, unitIndex, content, basePath }: Props) {
  ...
  return (
    <section className="min-h-screen bg-white relative" style={{ overflowX: "hidden" }}>
      ...
      <MobileUnitNav subject={subject} unit={unit} prevUnit={prevUnit} nextUnit={nextUnit} basePath={basePath} printRef={printRef} />
            ...
            <div className={`relative rounded-2xl overflow-hidden mb-4 sm:mb-5 bg-gradient-to-r ${GRAD} shadow-lg`}>
              ...
              <div className="relative z-10 px-4 py-5 sm:px-7 sm:py-7">
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {[`Unit ${unitIndex + 1} of ${subject.units.length}`, `${unit.readTime} min`, unit.difficulty].map((t) => (
                ...
                {content ? (
                  ...
                  <div className="py-16 text-center px-4">
                    ...
                    <p className="text-gray-400 text-xs mb-4 break-all">
                      Place <code className="bg-gray-100 px-1.5 py-0.5 rounded text-blue-600 font-mono">public/content/{unit.contentFile}</code>
                    </p>
                    <Link href={basePath} className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r ${GRAD} text-white text-xs font-extrabold`}>← Back to all units</Link>
                  </div>
                )}
              </div>
            </div>
