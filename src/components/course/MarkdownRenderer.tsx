...
const Zoom = ImageZoom as unknown as ComponentType<{ src: string; alt?: string; children: ReactNode }>;
...
const mdComponents = {
    h1: ({ children }: any) => (
        <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold bg-gradient-to-r from-blue-600 to-green-500 bg-clip-text text-transparent mt-8 mb-4 pb-3 border-b-2 border-blue-100 break-words">
            {children}
        </h1>
    ),
    h2: ({ children }: any) => (
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mt-8 mb-4 pb-2 border-b border-gray-200 break-words">
            {children}
        </h2>
    ),
    h3: ({ children }: any) => (
        <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-blue-600 to-green-500 bg-clip-text text-transparent mt-6 mb-3 break-words">
            {children}
        </h3>
    ),
    h4: ({ children }: any) => (
    ...
    p: ({ children }: any) => (
        <p className="text-gray-700 leading-relaxed text-sm sm:text-base mb-4 break-words">{children}</p>
    ),
    ul: ({ children }: any) => <ul className="space-y-2 mb-4 pl-1">{children}</ul>,
    ol: ({ children }: any) => (
        <ol className="space-y-2 mb-4 pl-4 list-decimal text-gray-700 text-sm sm:text-base">{children}</ol>
    ),
    li: ({ children }: any) => (
    ...
    blockquote: ({ children }: any) => (
        <blockquote className="border-l-4 border-blue-400 bg-blue-50/60 px-4 py-3 rounded-r-xl my-4 text-gray-700 text-sm sm:text-base italic break-words">
            {children}
        </blockquote>
    ),
